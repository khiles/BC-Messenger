...
// Reactions are one row per member so a message can carry multiple emoji and a
// per-emoji "who reacted" list. The original schema used message_ref as the
// primary key with a single set_by; migrate it in place when detected.
const oldReactionTable = db.prepare(
  "SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'message_reactions'"
).get();
if (oldReactionTable) {
  const hasSetBy = db.prepare(
    "SELECT 1 FROM pragma_table_info('message_reactions') WHERE name = 'set_by'"
  ).get();
  if (hasSetBy) {
    db.exec(`
      ...
      INSERT OR IGNORE INTO message_reactions_v2 (message_ref, member_number, emoji, reacted_at)
        SELECT message_ref, set_by, emoji, reacted_at FROM message_reactions;
...
db.exec('CREATE INDEX IF NOT EXISTS idx_reactions_msg ON message_reactions(message_ref);');
...
// Group join-by-code: invite tokens admins create, and pending join requests
// from non-members who want in. Tokens and requests cascade with their group.
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS idx_group_join_req ON group_join_requests(group_id, status);
`);
