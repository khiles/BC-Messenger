...
// Load .env (GIPHY_API_KEY, PORT, ...) if present — no external dependency.
try {
...
const PACK_DIR    = path.join(__dirname, 'pack');
for (const dir of [UPLOADS_DIR, PACK_DIR]) {
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
}

const MAX_STICKER_BYTES = 2 * 1024 * 1024;
const MAX_JOIN_REQUEST_NOTE_LENGTH = 300;
...
// Sticker / pack media is served straight from disk
app.use('/uploads', express.static(UPLOADS_DIR));
app.use('/pack', express.static(PACK_DIR));

// The public origin clients use to build absolute URLs for uploaded media and
// join links. `trust proxy` makes req.protocol/host reflect the nginx origin.
function relayOrigin(req) {
...
function listAccessibleReactions(memberNumber) {
  return db.prepare(`
    SELECT mr.message_ref, mr.emoji, mr.member_number
...
// Collapse raw reaction rows into the wire shape used by /api/state and WS
// pushes: { [messageRef]: { [emoji]: [memberNumbers] } }.
function groupReactionsByRef(rows) {
  const out = {};
  for (const r of rows) {
    if (!r?.message_ref || !r?.emoji) continue;
    const ref   = String(r.message_ref);
    const emoji = String(r.emoji);
    if (!out[ref]) out[ref] = {};
    if (!out[ref][emoji]) out[ref][emoji] = [];
    if (r.member_number) out[ref][emoji].push(Number(r.member_number));
  }
  return out;
}

function getReactionSetForRef(messageRef) {
  const rows = db.prepare(
    'SELECT message_ref, emoji, member_number FROM message_reactions WHERE message_ref = ? ORDER BY reacted_at ASC, rowid ASC'
  ).all(messageRef);
  return groupReactionsByRef(rows)[String(messageRef)] || {};
}
...
app.get('/api/state', authMiddleware, (req, res) => {
  ...
  const reactions = groupReactionsByRef(reactionRows);
...
app.post('/api/reactions', authMiddleware, (req, res) => {
  ...
  const myNum  = req.memberNumber;
  const msgRef = resolved.messageRef;

  // One reaction per member: clear any previous reaction, then set the new one.
  db.prepare('DELETE FROM message_reactions WHERE message_ref = ? AND member_number = ?')
    .run(msgRef, myNum);
  if (emoji) {
    db.prepare(`
      INSERT INTO message_reactions (message_ref, member_number, emoji, reacted_at)
      VALUES (?, ?, ?, ?)
    `).run(msgRef, myNum, emoji, Date.now());
  }

  const reactions = getReactionSetForRef(msgRef);
  const payload = {
  ...
  res.json({ success: true, messageRef: msgRef, memberNumber: myNum, emoji, reactions });
});
...
// ── Group join-by-code routes ────────────────────────────────────────────────

function getGroupInviteByToken(token) {
  return db.prepare('SELECT * FROM group_invites WHERE token = ?').get(token);
}

function isGroupInviteValid(invite) {
  if (!invite) return false;
  if (invite.expires_at && Number(invite.expires_at) <= Date.now()) return false;
  return true;
}

function notifyGroupAdmins(groupId, payload) {
  const admins = db.prepare('SELECT member_number FROM group_members WHERE group_id = ? AND role = ?').all(groupId, 'admin');
  for (const a of admins) {
    const ws = clients.get(a.member_number);
    if (ws && ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify(payload));
  }
}

// Create (or rotate) a join code for a group — admin only
app.post('/api/groups/:groupId/invite', authMiddleware, (req, res) => {
  const groupId = parseInt(req.params.groupId, 10);
  try {
    const membership = db.prepare('SELECT role FROM group_members WHERE group_id = ? AND member_number = ?').get(groupId, req.memberNumber);
    if (!membership) return res.status(403).json({ error: 'Not a member of this group' });
    if (membership.role !== 'admin') return res.status(403).json({ error: 'Only admins can create a join link' });
    ...
    db.prepare('INSERT INTO group_invites (token, group_id, created_by, created_at) VALUES (?, ?, ?, ?)')
      .run(token, groupId, req.memberNumber, Date.now());
    const url = `${relayOrigin(req)}/j/${token}`;
    res.json({
      success: true,
      token,
      url,
      groupName: group.name,
      inviteText: `Join my BCM group "${group.name}"! Open BC Messenger → ⋯ → "Join group by code" and enter: ${token}`,
    });
  } catch (err) {
...
// Public preview of a group behind a join code
app.get('/api/groups/join/:token', (req, res) => {
  const invite = getGroupInviteByToken(String(req.params.token || '').trim());
  if (!isGroupInviteValid(invite)) return res.status(404).json({ error: 'Invalid or expired join code' });
  const group = db.prepare('SELECT id, name, avatar_color, created_by FROM groups WHERE id = ?').get(invite.group_id);
  if (!group) return res.status(404).json({ error: 'Group no longer exists' });
  const memberCount = db.prepare('SELECT COUNT(*) AS c FROM group_members WHERE group_id = ?').get(group.id)?.c ?? 0;
  const creator = db.prepare('SELECT member_name FROM users WHERE member_number = ?').get(group.created_by);
  res.json({
    token: invite.token,
    groupId: group.id,
    groupName: group.name,
    ...
    memberCount,
    createdByName: creator?.member_name || `Member #${group.created_by}`,
  });
});

// Request to join a group via a join code
app.post('/api/groups/join/:token', authMiddleware, (req, res) => {
  const invite = getGroupInviteByToken(String(req.params.token || '').trim());
  if (!isGroupInviteValid(invite)) return res.status(404).json({ error: 'Invalid or expired join code' });
  const group = db.prepare('SELECT id, name FROM groups WHERE id = ?').get(invite.group_id);
  if (!group) return res.status(404).json({ error: 'Group no longer exists' });

  const alreadyMember = db.prepare('SELECT 1 FROM group_members WHERE group_id = ? AND member_number = ?').get(group.id, req.memberNumber);
  if (alreadyMember) {
    return res.json({ success: true, alreadyMember: true, groupId: group.id, groupName: group.name });
  }

  const existing = db.prepare('SELECT id, status FROM group_join_requests WHERE group_id = ? AND member_number = ?').get(group.id, req.memberNumber);
  if (existing?.status === 'pending') {
    return res.json({ success: true, pending: true, requestId: existing.id, groupId: group.id, groupName: group.name });
  }
  // Replace a prior declined request so the member can re-apply
  if (existing) db.prepare('DELETE FROM group_join_requests WHERE id = ?').run(existing.id);

  const note = String(req.body?.note ?? '').trim().slice(0, MAX_JOIN_REQUEST_NOTE_LENGTH);
  const now  = Date.now();
  const result = db.prepare(
    'INSERT INTO group_join_requests (group_id, member_number, note, status, requested_at) VALUES (?, ?, ?, \'pending\', ?)'
  ).run(group.id, req.memberNumber, note, now);

  notifyGroupAdmins(group.id, {
  ...
  res.json({ success: true, pending: true, requestId: result.lastInsertRowid, groupId: group.id, groupName: group.name });
});

// Pending join requests for a group — admin only
app.get('/api/groups/:groupId/join-requests', authMiddleware, (req, res) => {
  const groupId = parseInt(req.params.groupId, 10);
  const membership = db.prepare('SELECT role FROM group_members WHERE group_id = ? AND member_number = ?').get(groupId, req.memberNumber);
  if (!membership) return res.status(403).json({ error: 'Not a member of this group' });
  if (membership.role !== 'admin') return res.status(403).json({ error: 'Only admins can view join requests' });
  const rows = db.prepare(`
    SELECT jr.id, jr.group_id, jr.member_number, jr.note, jr.requested_at, u.member_name
    FROM group_join_requests jr
    LEFT JOIN users u ON u.member_number = jr.member_number
...
app.post('/api/groups/:groupId/join-requests/:id/accept', authMiddleware, (req, res) => {
  const groupId   = parseInt(req.params.groupId, 10);
  const requestId = parseInt(req.params.id, 10);
  try {
    const membership = db.prepare('SELECT role FROM group_members WHERE group_id = ? AND member_number = ?').get(groupId, req.memberNumber);
    if (!membership) return res.status(403).json({ error: 'Not a member of this group' });
    if (membership.role !== 'admin') return res.status(403).json({ error: 'Only admins can accept join requests' });
    const jr = db.prepare('SELECT * FROM group_join_requests WHERE id = ? AND group_id = ? AND status = ?').get(requestId, groupId, 'pending');
    if (!jr) return res.status(404).json({ error: 'Join request not found' });
    const group = db.prepare('SELECT name, avatar_color FROM groups WHERE id = ?').get(groupId);

    const now = Date.now();
    db.prepare('UPDATE group_join_requests SET status = ?, reviewed_at = ?, reviewed_by = ? WHERE id = ?')
      .run('accepted', now, req.memberNumber, requestId);
    db.prepare('INSERT OR IGNORE INTO group_members (group_id, member_number, joined_at, role) VALUES (?, ?, ?, \'member\')')
      .run(groupId, jr.member_number, now);

    const fullMembers = db.prepare('SELECT member_number, role FROM group_members WHERE group_id = ? ORDER BY joined_at ASC').all(groupId);
    const allMembers  = db.prepare('SELECT member_number FROM group_members WHERE group_id = ?').all(groupId);
    for (const m of allMembers) {
      const ws = clients.get(m.member_number);
      if (ws && ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type: 'group_members_updated', groupId, members: fullMembers }));
    }
    const requesterWs = clients.get(jr.member_number);
...
app.post('/api/groups/:groupId/join-requests/:id/decline', authMiddleware, (req, res) => {
  const groupId   = parseInt(req.params.groupId, 10);
  const requestId = parseInt(req.params.id, 10);
  try {
    const membership = db.prepare('SELECT role FROM group_members WHERE group_id = ? AND member_number = ?').get(groupId, req.memberNumber);
    if (!membership) return res.status(403).json({ error: 'Not a member of this group' });
    if (membership.role !== 'admin') return res.status(403).json({ error: 'Only admins can decline join requests' });
    const jr = db.prepare('SELECT * FROM group_join_requests WHERE id = ? AND group_id = ? AND status = ?').get(requestId, groupId, 'pending');
    if (!jr) return res.status(404).json({ error: 'Join request not found' });
    const group = db.prepare('SELECT name, avatar_color FROM groups WHERE id = ?').get(groupId);

    db.prepare('UPDATE group_join_requests SET status = ?, reviewed_at = ?, reviewed_by = ? WHERE id = ?')
      .run('declined', Date.now(), req.memberNumber, requestId);
...
// Tiny landing page so shared join links aren't dead ends
app.get('/j/:token', (req, res) => {
  const invite = getGroupInviteByToken(String(req.params.token || '').trim());
  const valid = isGroupInviteValid(invite);
  res.setHeader('Content-Type', 'text/html; charset=utf-8');
  res.send(`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>BC Messenger — Group invite</title>
  <style>
    body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#0f0d14;color:#e8e0f0;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:24px;margin:0}
    .card{background:#1a1626;border:1px solid #2e2540;border-radius:16px;padding:40px;max-width:440px;width:100%;text-align:center}
    .icon{font-size:48px;margin-bottom:14px}
    h1{font-size:20px;margin:0 0 8px}
    p{font-size:13px;color:#9b85b8;line-height:1.6;margin:0 0 18px}
    .code{background:#241d35;border:1px solid #2e2540;border-radius:10px;padding:12px 16px;font-size:15px;font-family:monospace;color:#e8e0f0;letter-spacing:1px;user-select:all;word-break:break-all}
  </style>
</head>
<body>
  <div class="card">
    <div class="icon">👥</div>
    <h1>Group invite</h1>
    ${valid ? `<p>This code invites you to join a BC Messenger group. Copy it, then in Bondage Club open BC Messenger → <strong>⋯</strong> → <strong>Join group by code</strong> and paste it in.</p>
    <div class="code">${String(invite.token)}</div>` : '<p>This invite code is invalid or has expired. Ask a group admin for a fresh one.</p>'}
  </div>
</body>
</html>`);
});
...
app.delete('/api/account/data', authMiddleware, (req, res) => {
  ...
  // 5. Delete reactions (DM and group, plus any the user placed)
  ...
  safeRun('message_reactions (own)', 'DELETE FROM message_reactions WHERE member_number = ?', member);
  ...
  safeRun('group_join_requests', 'DELETE FROM group_join_requests WHERE member_number = ?', member);
  safeRun('group_invites (created)', 'DELETE FROM group_invites WHERE created_by = ?', member);
...
// Upload a sticker (PNG/GIF/WebP) — stored on disk and served from /uploads
app.post('/api/uploads/sticker', authMiddleware, (req, res) => {
  ...
  if (buf.length > MAX_STICKER_BYTES) {
    return res.status(400).json({ error: `Sticker must be ${MAX_STICKER_BYTES / 1024 / 1024}MB or smaller` });
  }
  const ext = detectImageExt(buf);
  if (!ext) return res.status(400).json({ error: 'Only PNG, GIF, or WebP images are supported' });

  const safeName = String(name ?? '').trim().slice(0, 60).replace(/[^\w\- ]/g, '').replace(/\s+/g, '_') || 'sticker';
  const filename = `${req.memberNumber}-${Date.now()}-${safeName}.${ext}`;
  try {
  ...
  res.json({ success: true, url: `${relayOrigin(req)}/uploads/${filename}` });
});

// List built-in pack stickers plus the caller's uploads
app.get('/api/stickers', authMiddleware, (req, res) => {
  const pack = [];
  try {
    for (const f of fs.readdirSync(PACK_DIR)) {
      if (!/\.(png|gif|webp)$/i.test(f)) continue;
      pack.push({ url: `${relayOrigin(req)}/pack/${f}`, name: f.replace(/\.[^.]+$/, ''), owner: 0 });
    }
  } catch {}
  const mine = [];
  try {
    for (const f of fs.readdirSync(UPLOADS_DIR)) {
      if (!/\.(png|gif|webp)$/i.test(f)) continue;
      if (!f.startsWith(`${req.memberNumber}-`)) continue;
      mine.push({
        url: `${relayOrigin(req)}/uploads/${f}`,
        name: f.replace(/\.[^.]+$/, '').replace(/^\d+-\d+-/, ''),
        owner: req.memberNumber,
      });
    }
  } catch {}
  mine.sort((a, b) => a.name.localeCompare(b.name));
  res.json({ stickers: [...pack, ...mine] });
});

// Optional GIF search — proxied through Giphy so the API key stays server-side.
// Disabled (400) unless GIPHY_API_KEY is configured.
app.get('/api/gif-search', authMiddleware, async (req, res) => {
  const key = process.env.GIPHY_API_KEY;
  if (!key) return res.status(400).json({ error: 'GIF search is not enabled on this server' });
  ...
  try {
    ...
    const resp = await fetch(`https://api.giphy.com/v1/gifs/search?${new URLSearchParams({ api_key: key, q, limit, rating: 'g' })}`, {
      signal: controller.signal,
    });
    clearTimeout(tid);
    if (!resp.ok) return res.status(502).json({ error: 'GIF search failed', detail: resp.status });
    const data = await resp.json();
    const results = (data.data || []).map(g => {
      const img = g.images || {};
      const send = img.fixed_height || img.original || {};
      const prev = img.fixed_height_small || img.preview_gif || send;
      return {
        url: send.url || '',
        previewUrl: prev.url || send.url || '',
