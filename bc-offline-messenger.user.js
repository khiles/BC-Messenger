// ==UserScript==
// @name         BC Offline Messenger
// @namespace    https://github.com/khiles/BC-Messenger
// @version      1.1.0
// @description  Private messenger for Bondage Club with offline message delivery and group messaging
// @author       BC-Messenger
// @match        https://www.bondageprojects.elementfx.com/*
// @match        https://bondageprojects.elementfx.com/*
// @match        https://www.bondageprojects.com/*
// @match        https://bondageprojects.com/*
// @updateURL    https://raw.githubusercontent.com/khiles/BC-Messenger/main/bc-offline-messenger.user.js
// @downloadURL  https://raw.githubusercontent.com/khiles/BC-Messenger/main/bc-offline-messenger.user.js
// @connect      messenger.vo-p.com
...
(() => {
  // src/constants.js
  var SERVER = "https://messenger.vo-p.com";
  var WS_URL = "wss://messenger.vo-p.com/ws";
  var SCRIPT_VERSION = "1.2.0";
  var UPDATE_URL = "https://raw.githubusercontent.com/khiles/BC-Messenger/main/bc-offline-messenger.user.js";
  var STORE_BASE = "bcm_";
  var DB_NAME = "BCOfflineMessenger";
  var DB_VER = 5;
  var DEFAULT_AWAY_TIMEOUT_MINS = 10;
  var MIN_AWAY_TIMEOUT_MINS = 1;
  var MAX_AWAY_TIMEOUT_MINS = 240;
  var MIN_ACTIVITY_DEBOUNCE_MS = 1e3;
  var MAX_STICKER_UPLOAD_BYTES = 2 * 1024 * 1024;
  var MAX_NOTIFICATION_BODY_LENGTH = 160;
  var MAX_QUOTE_TEXT_LENGTH = 200;
  var MAX_STATUS_LENGTH = 60;
  var MAX_DISCORD_WEBHOOK_LENGTH = 500;
  var MAX_REPORT_HISTORY_ITEMS = 30;
  var MAX_STARRED_PANEL_ITEMS = 300;
  var CONTEXT_MENU_ITEM_HEIGHT = 32;
  var CONTEXT_MENU_TOTAL_VERTICAL_PADDING = 12;
  var INITIAL_RECONNECT_RETRY_MS = 15e3;
  var MAX_RECONNECT_RETRY_MS = 12e4;
  var LOCAL_SEND_BYPASS_TTL_MS = 5e3;
  var MAX_LOCAL_SEND_BYPASS_KEYS = 64;
  var BCM_QUOTE_PREFIX = "[BCMQUOTE]";
  var BCM_MSG_PREFIX = "[BCMMSG]";
  var E2E_PREFIX = "[BCME2E]";
  var E2E_V2_PREFIX = "[BCME2Ev2]";
  var E2E_V2G_PREFIX = "[BCME2Ev2g]";
  var VIRT_PAGE_SIZE = 80;
  var DISAPPEAR_OPTIONS = [
    { value: 0, label: "Off" },
    { value: 60 * 60 * 1e3, label: "1h" },
    { value: 6 * 60 * 60 * 1e3, label: "6h" },
    { value: 24 * 60 * 60 * 1e3, label: "24h" },
    { value: 7 * 24 * 60 * 60 * 1e3, label: "7d" }
  ];
  ...
  function openModal({ title, body, buttons = [] }) {
    return new Promise((resolve) => {
      ...
      for (const b of btns) {
        ...
        btnRow.appendChild(el("button", { cls, type: "button", onclick: () => {
          if (b.onClick) b.onClick();
          finish(b.value ?? (b.primary ? true : false));
        } }, b.label));
      }
  ...
  function openSelect(message, options) {
    return new Promise((resolve) => {
      const finish = (val) => {
        overlay.remove();
        resolve(val);
  ...
  // src/themes.js
  var STATIC_THEMES = {
    light: `--bcm-accent:#c43060;--bcm-accent-h:#a02050;--bcm-accent-bg:#fde8f0;--bcm-gradient:linear-gradient(135deg,#c43060,#e05888);--bcm-gradient-h:linear-gradient(135deg,#d44070,#ee6898);--bcm-bg:#ffffff;--bcm-bg-side:#f8f9fb;--bcm-bg-title:#f4f5f7;--bcm-bg-input:#ffffff;--bcm-text:#2d2d2d;--bcm-text-muted:#8890a0;--bcm-border:#e2e4e9;--bcm-bubble-sent:#fce8f0;--bcm-bubble-sent-b:#f0b0c8;--bcm-bubble-recv:#f0f2f6;--bcm-bubble-recv-b:#e2e4e9;--bcm-online:#34c468;--bcm-font-size:13px`,
    dark: `--bcm-accent:#e05888;--bcm-accent-h:#f07898;--bcm-accent-bg:#3a1828;--bcm-gradient:linear-gradient(135deg,#c43060,#e05888);--bcm-gradient-h:linear-gradient(135deg,#d44070,#ee6898);--bcm-bg:#1e1e2e;--bcm-bg-side:#181828;--bcm-bg-title:#141420;--bcm-bg-input:#252538;--bcm-text:#e0e0f0;--bcm-text-muted:#7878a0;--bcm-border:#2e2e4e;--bcm-bubble-sent:#3a1828;--bcm-bubble-sent-b:#6a2848;--bcm-bubble-recv:#252538;--bcm-bubble-recv-b:#2e2e4e;--bcm-online:#34c468;--bcm-font-size:13px`,
    midnight: `--bcm-accent:#5090e0;--bcm-accent-h:#70b0ff;--bcm-accent-bg:#0a1828;--bcm-gradient:linear-gradient(135deg,#3070c0,#5090e0);--bcm-gradient-h:linear-gradient(135deg,#4080d0,#60a0f0);--bcm-bg:#000000;--bcm-bg-side:#0a0a0a;--bcm-bg-title:#050505;--bcm-bg-input:#0f0f1a;--bcm-text:#d0d8e8;--bcm-text-muted:#506080;--bcm-border:#1a1a2a;--bcm-bubble-sent:#0a1828;--bcm-bubble-sent-b:#1a3050;--bcm-bubble-recv:#0f0f1a;--bcm-bubble-recv-b:#1a1a2a;--bcm-online:#30d468;--bcm-font-size:13px`,
    lavender: `--bcm-accent:#7c5cbf;--bcm-accent-h:#9370d8;--bcm-accent-bg:#f0ebff;--bcm-gradient:linear-gradient(135deg,#7c5cbf,#a07ad8);--bcm-gradient-h:linear-gradient(135deg,#8c6ccf,#b08ae8);--bcm-bg:#ffffff;--bcm-bg-side:#f6f4ff;--bcm-bg-title:#f0eeff;--bcm-bg-input:#ffffff;--bcm-text:#2d2040;--bcm-text-muted:#8880a8;--bcm-border:#dcd8ee;--bcm-bubble-sent:#ede8ff;--bcm-bubble-sent-b:#c8b8f0;--bcm-bubble-recv:#f0f0f8;--bcm-bubble-recv-b:#dcd8ee;--bcm-online:#34c468;--bcm-font-size:13px`
  };
  function buildCustomTheme(storePrefix) {
    const c = (k, d) => GM_getValue(storePrefix + "custom_" + k, d);
    return `--bcm-accent:${c("accent", "#c43060")};--bcm-accent-h:${c("accent_h", "#a02050")};--bcm-accent-bg:${c("accent_bg", "#fde8f0")};--bcm-gradient:${c("gradient", "linear-gradient(135deg,#c43060,#e05888)")};--bcm-gradient-h:${c("gradient_h", "linear-gradient(135deg,#d44070,#ee6898)")};--bcm-bg:${c("bg", "#ffffff")};--bcm-bg-side:${c("bg_side", "#f8f9fb")};--bcm-bg-title:${c("bg_title", "#f4f5f7")};--bcm-bg-input:${c("bg_input", "#ffffff")};--bcm-text:${c("text", "#2d2d2d")};--bcm-text-muted:${c("text_muted", "#8890a0")};--bcm-border:${c("border", "#e2e4e9")};--bcm-bubble-sent:${c("bubble_sent", "#fce8f0")};--bcm-bubble-sent-b:${c("bubble_sent_b", "#f0b0c8")};--bcm-bubble-recv:${c("bubble_recv", "#f0f2f6")};--bcm-bubble-recv-b:${c("bubble_recv_b", "#e2e4e9")};--bcm-online:${c("online", "#34c468")};--bcm-font-size:13px`;
  }

  // src/api.js
  var authHeaderProvider = () => "";
  function setAuthHeaderProvider(fn) {
  ...
  function httpPost(path, body, auth = true) {
  ...
  function httpGet(path) {
    return new Promise((resolve, reject) => {
      GM_xmlhttpRequest({
        method: "GET",
        url: SERVER + path,
        headers: { "Authorization": authHeaderProvider() },
        timeout: 1e4,
        onload: (r) => {
          try {
            resolve(parseResponse(r));
  ...
  function httpPut(path, body) {
    return new Promise((resolve, reject) => {
      GM_xmlhttpRequest({
        method: "PUT",
        url: SERVER + path,
        headers: { "Content-Type": "application/json", "Authorization": authHeaderProvider() },
        timeout: 1e4,
        data: JSON.stringify(body),
        onload: (r) => {
          try {
  ...
  var registerFn = async () => false;
  function setRegisterFn(fn) {
    registerFn = fn;
  }
  ...
  function fetchLinkPreview(url) {
    return new Promise((resolve) => {
      GM_xmlhttpRequest({
        method: "GET",
        url: `${SERVER}/api/link-preview?url=${encodeURIComponent(url)}`,
        headers: { "Authorization": authHeaderProvider() },
        timeout: 8e3,
        onload: (r) => {
          try {
  ...
  function uploadSticker(dataBase64, name = "") {
  ...
  function acceptGroupJoinRequest(groupId, requestId) {
    return withAuthRetry(() => httpPost(`/api/groups/${groupId}/join-requests/${requestId}/accept`, {}));
  }
  function declineGroupJoinRequest(groupId, requestId) {
  ...
  async function exportE2EPubKey(kp) {
    const raw = await crypto.subtle.exportKey("raw", kp.publicKey);
    return btoa(String.fromCharCode(...new Uint8Array(raw)));
  }
  async function exportE2EPrivKey(kp) {
    return JSON.stringify(await crypto.subtle.exportKey("jwk", kp.privateKey));
  }
  async function importE2EPubKey(b64) {
    const raw = Uint8Array.from(atob(b64), (c) => c.charCodeAt(0));
    return crypto.subtle.importKey("raw", raw, { name: "ECDH", namedCurve: "P-256" }, false, []);
  }
  async function importE2EPrivKey(jwkStr) {
  ...
  async function deriveE2ESharedKey(privKey, theirPubKey) {
  ...
  async function encryptE2E(sharedKey, plaintext, aad = null) {
  ...
  async function decryptE2E(sharedKey, encrypted, aad = null) {
  ...
  function dmAAD(senderNum, recipientNum) {
    return new TextEncoder().encode(`bcm-v2|${senderNum}|${recipientNum}`);
  }
  function groupAAD(groupId, senderNum) {
    return new TextEncoder().encode(`bcm-v2g|${groupId}|${senderNum}`);
  }
  async function computeSafetyNumber(myPubB64, theirPubB64) {
  ...
  var E2E_V2G_PREFIX2 = "[BCME2Ev2g]";
  var e2ePrivateKey = null;
  var e2ePublicKeyB64 = null;
  var e2eSharedKeys = /* @__PURE__ */ new Map();
  var e2eContactPubKeys = /* @__PURE__ */ new Map();
  var e2ePinned = {};
  var e2eAllowPlaintext = {};
  var e2eKeyChanged = /* @__PURE__ */ new Set();
  var getStore = () => "bcm_default_";
  var getMemberNumber = () => null;
  function initCryptoContext({ getStore: gs, getMemberNumber: gm } = {}) {
    if (gs) getStore = gs;
  ...
  function loadPinnedE2E() {
    ...
    try {
      e2ePinned = JSON.parse(GM_getValue(STORE + "e2e_pinned", "{}")) || {};
    } catch {
      e2ePinned = {};
    }
    try {
      e2eAllowPlaintext = JSON.parse(GM_getValue(STORE + "e2e_allow_plain", "{}")) || {};
    } catch {
      e2eAllowPlaintext = {};
    }
  }
  ...
  async function getContactSharedKey(memberNum) {
    ...
    if (!e2eContactPubKeys.has(memberNum)) {
      try {
        ...
        e2eContactPubKeys.set(memberNum, null);
      }
    }
  ...
  async function acceptKeyChange(memberNum) {
    memberNum = Number(memberNum);
  ...
  function markContactVerified(memberNum, verified = true) {
    memberNum = Number(memberNum);
  ...
  function getPinnedFor(memberNum) {
    return e2ePinned[Number(memberNum)] ?? null;
  }
  function getAllowPlaintext(memberNum) {
    return !!e2eAllowPlaintext[Number(memberNum)];
  }
  function setAllowPlaintext(memberNum, allow) {
    e2eAllowPlaintext[Number(memberNum)] = !!allow;
    saveAllowPlaintext();
  }
  async function safetyNumberFor(memberNum) {
    return computeSafetyNumber(e2ePublicKeyB64, e2ePinned[Number(memberNum)]?.pub ?? null);
  }
  async function decryptGroupContent(content, senderNum, groupId) {
    if (!content?.startsWith(E2E_V2G_PREFIX2)) return content;
    ...
    if (!key) return "[\u{1F512} Encrypted group message \u2014 no key for sender]";
    try {
      return await decryptE2E(key, content.slice(E2E_V2G_PREFIX2.length), aad);
    } catch {
      return "[\u{1F512} Could not decrypt group message]";
    }
  }
  ...
  var getActiveDbName = () => "BCOfflineMessenger_default";
  function initStorageContext({ getActiveDbName: gn } = {}) {
  ...
  function ensureDbReady() {
    ...
    dbOpenPromise = dbOpenPromise || openDB(unsafeWindow.indexedDB, getActiveDbName(), DB_VER).then((db) => {
      idbDb = db;
      return db;
    }).catch((err) => {
      dbOpenPromise = null;
  ...
  function getMessages(partnerNum) {
    return new Promise((resolve, reject) => {
  ...
  function deleteConversation(contactNum) {
    return new Promise((resolve) => {
      if (!idbDb) return resolve();
  ...
  function updateMsgStatus(serverId, status, readAt = null) {
  ...
  function updateMessageServerId(msg, serverId) {
  ...
  function findDuplicateLocalMessage(partnerNum, senderNum, content, sentAt) {
    return ensureDbReady().then((db) => new Promise((resolve) => {
      const tx = db.transaction("messages", "readonly");
      ...
      req.onsuccess = () => {
        ...
        const found = (req.result || []).find(
          (m) => m.senderNum === senderNum && m.content === content && m.serverId == null && !m.fromUs && Math.abs((Number(m.sentAt) || 0) - ts) < tol
        );
        resolve(found || null);
      };
      req.onerror = () => resolve(null);
  ...
  function getSyncCursor(key = "default") {
    return ensureDbReady().then((db) => new Promise((resolve) => {
  ...
  function setSyncCursor(cursor, key = "default") {
    return ensureDbReady().then((db) => new Promise((resolve) => {
  ...
  // src/bc-integration.js
  function isMemberNumberLikeName(name, memberNum = null) {
  ...
  function getSafeDisplayName(name, memberNum = null, fallback = "") {
    const raw = String(name ?? "").trim();
    if (!raw || isMemberNumberLikeName(raw, memberNum)) return fallback;
    return raw;
  }
  function getFriendName(num) {
  ...
  function iterateFriendNames(callback) {
    const fn = unsafeWindow.Player?.FriendNames;
    if (!fn) return;
  ...
  function coerceOnlineFlag(value) {
  ...
  function getFriendEntryOnlineFlag(friend) {
    if (!friend || typeof friend !== "object") return null;
    const direct = coerceOnlineFlag(friend.IsOnline) ?? coerceOnlineFlag(friend.isOnline) ?? coerceOnlineFlag(friend.Online) ?? coerceOnlineFlag(friend.online) ?? coerceOnlineFlag(friend.Connected) ?? coerceOnlineFlag(friend.connected) ?? coerceOnlineFlag(friend.Presence) ?? coerceOnlineFlag(friend.presence);
  ...
  function parseFriendMemberNumber(value) {
    const n = parseInt(value, 10);
    return Number.isFinite(n) && n > 0 ? n : 0;
  }
  function isCharacterLoadedForMember(num) {
    const n = Number(num);
    if (!n) return false;
  ...
  function parseBCFriendEntry(rawFriend) {
    if (rawFriend == null) return null;
    const friend = rawFriend && typeof rawFriend === "object" ? rawFriend.Member ?? rawFriend.member ?? rawFriend.Friend ?? rawFriend.friend ?? rawFriend : rawFriend;
    const num = friend && typeof friend === "object" ? parseFriendMemberNumber(
      friend.MemberNumber ?? friend.memberNumber ?? friend.MemberNum ?? friend.memberNum ?? friend.MemberID ?? friend.memberId ?? friend.Member ?? friend.member
    ) : parseFriendMemberNumber(friend);
    if (!num) return null;
    const name = friend && typeof friend === "object" ? friend.MemberName ?? friend.memberName ?? friend.Name ?? friend.name ?? friend.Nickname ?? friend.nickname ?? "" : "";
    const safeName = getSafeDisplayName(name, num, "");
    const online = getFriendEntryOnlineFlag(friend);
    const lastSeen = Number(
      friend && typeof friend === "object" ? friend.LastSeen ?? friend.lastSeen ?? friend.LastLogin ?? friend.lastLogin ?? friend.LastActivity ?? friend.lastActivity ?? 0 : 0
    ) || 0;
    const room = friend && typeof friend === "object" ? String(friend.ChatRoomName ?? friend.chatRoomName ?? friend.RoomName ?? friend.roomName ?? friend.Room ?? friend.room ?? "").trim() : "";
    return { memberNum: num, name: safeName, online, lastSeen, room };
  }
  function getBCFriendEntries() {
    const W = unsafeWindow;
  ...
  function memberAvailClass(num, isOnline) {
    if (!isOnline) return "";
  ...
  function isMemberOnlineForUi(num, serverOnline = null) {
    const n = Number(num);
    if (!n) return false;
  ...
  function normalizeGroupMembers(members) {
    return (Array.isArray(members) ? members : []).map((m) => {
      if (m && typeof m === "object") {
        const n2 = Number(m.member_number ?? m.memberNumber ?? m.memberNum ?? m.id);
        if (!n2) return null;
        return { member_number: n2, role: String(m.role || "member") };
      }
      const n = Number(m);
      if (!n) return null;
  ...
  async function sendToServer(recipientNum, content, alreadyDelivered = false, parentMessageRef = null, whisperDelivered = false) {
...
(OK = send once, Cancel = abort. Use the lock menu in the chat header to remember this choice for the contact.)`);
  ...
  async function sendGroupMessage(groupId, content, mentionTargets = [], parentMessageRef = null) {
    if (isE2EReady()) {
...
`;
  ...
  // src/utils.js
  function hashBypassContent(content) {
  ...
  function deriveStatusFromGroupReceipt(receipt) {
  ...
  function encodeMessagePayload(content, quote, options = {}) {
    const text = String(content ?? "");
  ...
  function isServerBackedMessageKey(key) {
  ...
  function loadAccountScopedSettings() {
    ...
    state.blockedMembers = /* @__PURE__ */ new Set();
    ...
    state.trustedContacts = /* @__PURE__ */ new Set();
  ...
  state.contactMeta = {};
  state.bcFriendCache = {};
  state.bcOnlineSet = /* @__PURE__ */ new Set();
  ...
  state.groupUnread = {};
  state.afkReplySent = /* @__PURE__ */ new Set();
  ...
  state.selectedMsgs = /* @__PURE__ */ new Set();
  ...
  function updateGroupMessageReceipt(groupMessageRef, senderMessageId, receipt) {
    ...
    return ensureDbReady().then((db) => new Promise((resolve) => {
      ...
      if (senderMessageId) {
        const req2 = st.index("serverId").get(senderMessageId);
        req2.onsuccess = () => {
          if (req2.result) return applyAndResolve(req2.result);
  ...
  async function getStatus(num) {
    try {
      return await httpGet(`/api/status/${num}`);
    } catch {
      return null;
    }
  }
  ...
  async function syncHistoryFromServer(limit = 500) {
    ...
    for (const m of rows) {
      ...
      if (body.startsWith(E2E_V2_PREFIX) || body.startsWith(E2E_PREFIX)) {
        const r2 = await getContactSharedKey(partnerNum).catch(() => ({ key: null }));
        if (r2?.key) {
          ...
          try {
            body = await decryptE2E(r2.key, ct, aad);
          } catch {
            body = "[\u{1F512} Could not decrypt message]";
          }
        } else {
          body = "[\u{1F512} Encrypted message \u2014 update your plugin to read this]";
        }
      }
  ...
  async function syncGroupHistoryFromServer(groupId, limit = 200) {
    ...
    for (const m of rows) {
      ...
      const status = fromUs ? deriveStatusFromGroupReceipt(receipt) : m?.read_at ? "read" : m?.delivered ? "delivered" : "sent";
  ...
  async function refreshBlockedMembersCache() {
    ...
    try {
      ...
      state.blockedMembers = new Set(
        (Array.isArray(result?.blocks) ? result.blocks : []).map((row) => Number(row?.blocked_number || 0)).filter((num) => num > 0)
      );
    } catch {
    }
    return state.blockedMembers;
  ...
  async function updateSyncCheckpoint(cursorAt) {
    try {
      await httpPost("/api/sync/checkpoint", { cursorAt });
    } catch {
    }
  }
  async function loadProfile(memberNum) {
    try {
      return await httpGet(`/api/profile/${memberNum}`);
    } catch {
      return null;
    }
  }
  ...
  async function getContactCard(memberNum) {
    try {
      return await withAuthRetry(() => httpGet(`/api/profile/${memberNum}/contact-card`));
    } catch {
      return null;
    }
  }
  ...
  async function getCollections() {
    try {
      return await withAuthRetry(() => httpGet("/api/collections"));
    } catch {
      return { collections: [] };
    }
  }
  async function getCollectionMessages(name) {
    try {
      return await httpGet(`/api/collections/${encodeURIComponent(name)}`);
    } catch {
      return { messages: [] };
    }
  }
  ...
  async function getPoll(messageRef) {
    try {
      return await httpGet(`/api/polls/${encodeURIComponent(messageRef)}`);
    } catch {
      return null;
    }
  }
  function connectWs() {
    ...
    try {
      ...
      console.error("[BCM] WS create error:", e);
      return;
    }
    state.ws.addEventListener("open", () => state.ws.send(JSON.stringify({ type: "auth", memberNumber: state.memberNumber, clientSecret: state.clientSecret })));
    state.ws.addEventListener("message", async (e) => {
      ...
      if (msg.type === "auth_ok") {
        ...
        console.error("[BCM] WS auth error");
        state.ws.close();
        ...
        await onIncomingMessage(msg.senderNumber, msg.senderName, msg.content, msg.sentAt, msg.id, msg.parentMessageRef);
        ...
        if (msg.messageRef) {
          if (msg.reactions && typeof msg.reactions === "object") {
            ...
            if (msg.emoji) (cur[_a = msg.emoji] || (cur[_a] = [])).push(msg.memberNumber);
        ...
        showNote(`\u{1F4E5} ${requesterName} requested to join "${msg.groupName || "your group"}"`, false);
        if (gid && state.selectedGroup === gid) {
          const manageBtn = state.dialogEl?.querySelector(".bcm-manage-group-btn");
          if (manageBtn) manageBtn.title = "Manage group \u2014 new join request";
        }
    ...
    state.ws.addEventListener("close", () => {
      state.ws = null;
      if (state.loggedIn) setTimeout(connectWs, 6e3);
    });
    ...
    try {
      ...
      if (content?.startsWith(E2E_V2_PREFIX)) {
        ...
        if (r?.key) {
          ...
          try {
            content = await decryptE2E(r.key, content.slice(E2E_V2_PREFIX.length), aad);
          } catch {
            content = "[\u{1F512} Could not decrypt message]";
          }
        } else if (r?.status === "changed") {
          content = "[\u{1F512} Encrypted \u2014 sender's key changed; verify before reading]";
        } else {
          content = "[\u{1F512} Encrypted message \u2014 update your plugin to read this]";
        }
      } else if (content?.startsWith(E2E_PREFIX)) {
        const r = await getContactSharedKey(parseInt(senderNum, 10)).catch(() => ({ key: null }));
        if (r?.key) {
          try {
            content = await decryptE2E(r.key, content.slice(E2E_PREFIX.length));
          } catch {
            content = "[\u{1F512} Could not decrypt message]";
          }
        } else {
      ...
      try {
        msg = await saveMessage(senderNum, senderNum, content, sentAt ?? Date.now(), false, serverId ?? null, null, false, parentMessageRef);
      } catch {
      }
      ...
      if (senderNum && senderNum !== state.memberNumber && state.autoResponderRules.length > 0) {
        ...
        for (const rule of state.autoResponderRules) {
          ...
          if (matches && rule.reply) {
            setTimeout(() => sendToServer(senderNum, rule.reply).catch(() => {
            }), 900 + Math.random() * 300);
            break;
          }
        }
      ...
      console.error("[BCM] onIncomingMessage:", e);
    }
  }
  function markMemberOffline(num) {
    ...
    if (state.loggedIn) {
      refreshContactList();
      try {
        unsafeWindow.ServerSend?.("AccountQuery", { Query: "FriendList" });
      } catch {
      }
    }
  }
  ...
  function recordOnlineNow(num) {
    ...
    buckets[(/* @__PURE__ */ new Date()).getHours()]++;
  ...
  async function sendBCMInvite(contactNum) {
    if (hasInvitedContact(contactNum)) {
      showNote("\u{1F4E8} Invite already sent to this contact", false);
      return;
    }
    ...
    let msg;
    try {
      msg = await saveMessage(contactNum, state.memberNumber, inviteText, now, true, null, "delivered");
    } catch {
    }
    if (msg && state.selectedContact === contactNum && !state.selectedGroup) {
      appendBubble(msg);
      scrollMsgs();
    }
    refreshContactList();
    ...
    showNote("\u{1F4E8} Invite sent!", false);
  }
  async function openSavedMessages() {
    if (!state.memberNumber) {
      showNote("Not logged in yet", true);
      return;
    }
    closeAllPanels();
    ...
    const hName = state.dialogEl?.querySelector(".bcm-msghead-name");
    if (hName) hName.textContent = "\u{1F4CC} Saved Messages";
  }
  async function sendFriendInvite(contactNum) {
    const name = state.memberName || `Member #${state.memberNumber}`;
    const text = `\u{1F44B} ${name} (#${state.memberNumber}) would like to add you as a BC friend!`;
    const now = Date.now();
    let msg;
    try {
      msg = await saveMessage(contactNum, state.memberNumber, text, now, true, null, "delivered");
    } catch {
    }
    if (msg && state.selectedContact === contactNum && !state.selectedGroup) {
      appendBubble(msg);
  ...
  async function sendRoomInvite(contactNum) {
    ...
    showNote("\u{1F3E0} Room invite sent!", false);
  }
  ...
  var INVITE_MARKER = "raw.githubusercontent.com/khiles/BC-Messenger";
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        for (const m of msgs) {
          ...
          if (Date.now() - Number(m.sentAt) >= 1728e5) continue;
  ...
  async function onIncomingGroupMessage(groupId, senderNum, senderName, content, sentAt, serverId, mentioned = false, mentionTargets = null, groupMessageRef = null, receipt = null) {
    try {
      ...
      let msg = {
        ...
        status: statusFromReceipt,
        messageType: "group",
        mentioned: isMentioned,
        groupMessageRef: groupMessageRef || null
      };
      msg = updateGroupReceiptForMessage(msg, receipt, groupMessageRef);
      try {
        msg = await saveGroupMessage(groupId, senderNum, senderName, content, sentAt ?? Date.now(), isFromUs, serverId ?? null, statusFromReceipt, false, groupMessageRef, receipt);
      } catch {
      }
      scheduleDisappearingForMessage(msg);
      const group = state.groups[groupId];
      if (group) {
        const displayName = isMentioned ? `\u{1F4AC} ${group.name} (you were mentioned)` : `${group.name}: ${senderName}`;
      ...
      console.error("[BCM] onIncomingGroupMessage:", e);
    }
  }
  ...
  function updateAwayIndicator() {
    ...
    if (title) title.textContent = state.isAway ? "\u{1F4AC} BC Messenger (AFK)" : "\u{1F4AC} BC Messenger";
  ...
  function scheduleAwayTimer() {
    ...
    const timeoutMs = state.awayTimeoutMins * 6e4;
  ...
  function registerAwayActivityTracking() {
    ...
    ["mousemove", "mousedown", "keydown", "touchstart"].forEach(
      (evt) => window.addEventListener(evt, onActivity, { passive: true, capture: true })
    );
    scheduleAwayTimer();
  }
  function loadModSDK() {
  ...
  function registerHooks(mod2) {
    mod2.hookFunction("ChatRoomMessageDisplay", 0, (args, next) => {
      const [data] = args;
      if (data?.Type === "ServerMessage" && (data.Content === "Beep" || data.Content === "Whisper")) {
        if (state.beepHideMode === 2) return;
        if (state.beepHideMode === 1 && state.dialogEl && document.body.contains(state.dialogEl)) return;
      }
      return next(args);
    });
    mod2.hookFunction("GameRun", 1, (args, next) => {
      const result = next(args);
      return result;
    });
    mod2.hookFunction("ServerSend", 1, async (args, next) => {
      try {
        const [type, data] = args;
        if (type === "ChatRoomJoin") {
          if (data?.Name) {
            const now = Date.now();
            if (state.lastChatRoomJoinSent.name === data.Name && now - state.lastChatRoomJoinSent.timestamp < 3e3) {
              return;
            }
              ...
              sendToServer(rn, data.Message, true).then((r) => {
                if (r?.success && r.id) {
                  ...
                  if (r.delivered === true && initialStatus !== "delivered") {
                    updateMsgStatus(r.id, "delivered").then((updated) => {
                      if (updated) updateBubbleTick(r.id, "delivered");
                    }).catch(() => {
                    });
                  }
                }
        ...
        console.error("[BCM] ServerSend hook:", e);
      }
      return next(args);
    });
    mod2.hookFunction("ServerAccountBeep", 0, (args, next) => {
      try {
        const [data] = args;
    ...
    try {
      mod2.hookFunction("ChatRoomMessage", 0, (args, next) => {
        try {
          const [data] = args;
          ...
          if (state.loggedIn && data?.Sender) {
            ...
            const isLeaveOrDisconnect = type === "Disconnect" || content.includes("disconnect") || content.includes("actionleave") || content.includes("actiondisconnect") || content.includes("serverleave") || content.includes("serverdisconnect");
    ...
    try {
      mod2.hookFunction("AccountQueryResult", 0, (args, next) => {
    ...
    try {
      mod2.hookFunction("FriendListLoadFriendList", 0, (args, next) => {
    ...
    try {
      mod2.hookFunction("InformationSheetRun", 1, (args, next) => {
        const result = next(args);
        const W = unsafeWindow;
    ...
    try {
      mod2.hookFunction("ChatRoomSync", 1, (args, next) => {
        const result = next(args);
        const W = unsafeWindow;
    ...
    try {
      mod2.hookFunction("ChatRoomLeave", 1, (args, next) => {
    ...
    try {
      mod2.hookFunction("CharacterDelete", 0, (args, next) => {
        const result = next(args);
        try {
          const raw = args[0];
  ...
  function processFriendListData(data) {
    const hasExplicitRowsArray = Array.isArray(data) || Array.isArray(data?.Result) || Array.isArray(data?.result) || Array.isArray(data?.Friends) || Array.isArray(data?.friends);
    if (!hasExplicitRowsArray) {
      return;
    }
    const rows = Array.isArray(data) ? data : Array.isArray(data?.Result) ? data.Result : Array.isArray(data?.result) ? data.result : Array.isArray(data?.Friends) ? data.Friends : Array.isArray(data?.friends) ? data.friends : [];
    ...
    const W = unsafeWindow;
    let changed = false;
    const returnedNums = /* @__PURE__ */ new Set();
    ...
    const allKnownNums = /* @__PURE__ */ new Set([
      ...
      if (argOne && typeof argOne === "object" && !Array.isArray(argOne) && ("Query" in argOne || "query" in argOne)) {
        ...
        if (query === "friendlist") {
          processFriendListData(
            argOne.Result ?? argOne.result ?? argOne.Data ?? argOne.data ?? argOne.Friends ?? argOne.friends
          );
        }
  ...
  function tryRegisterBCExtension() {
    ...
    W.PreferenceRegisterExtensionSetting({
      ...
      Draw: () => {
        ...
        W.DrawText("BC Offline Messenger", 1e3, 125, "#2d2d2d", "White");
        W.DrawText(`Member #${state.memberNumber ?? "\u2014"}  \xB7  ${connLabel}`, 1e3, 190, connColor, "");
        W.DrawButton(875, 280, 250, 55, "\u2699 Open Settings", "#fff0f4", "", "Open BC Messenger settings");
        W.DrawButton(875, 360, 250, 55, "\u{1F4AC} Open Messenger", "#f0f8ff", "", "Open BC Messenger window");
      },
      Click: () => {
  ...
  function scheduleBCExtensionRegistration() {
    if (!tryRegisterBCExtension()) setTimeout(scheduleBCExtensionRegistration, 2e3);
  }
  state.htmlTrigger = null;
  state.htmlBadgeEl = null;
  function applyTriggerBaseStyles(el2) {
    const s = el2.style;
  ...
  function showToast(senderName, content, senderNum) {
    ...
    if (matchedKw && !alwaysNotify) {
      ...
      Object.assign(kt.style, {
        ...
        color: "#2d2d2d",
        fontSize: "13px",
        boxShadow: "0 4px 20px rgba(240,176,0,.35)",
        cursor: "pointer",
        pointerEvents: "all",
        opacity: "0",
      ...
      kn.textContent = "\u{1F514} " + senderName + " \u2014 keyword: " + matchedKw;
      ...
      kt.appendChild(kb);
      kt.onclick = () => {
        if (senderNum && state.loggedIn) {
          openDialog();
          selectContact(senderNum, state.contactMeta[senderNum]?.name ?? `Member #${senderNum}`);
        }
        kt.remove();
      };
      root2.appendChild(kt);
      requestAnimationFrame(() => {
        kt.style.opacity = "1";
        kt.style.transform = "translateX(0)";
      });
      setTimeout(() => {
        kt.style.opacity = "0";
        kt.style.transform = "translateX(110%)";
        setTimeout(() => kt.remove(), 300);
      }, 6e3);
      return;
    }
    const inDnd = state.availabilityState === "dnd";
    const canNativeNotify = state.systemNotificationsEnabled && typeof Notification !== "undefined" && Notification.permission === "granted" && document.visibilityState === "hidden";
    ...
    Object.assign(toast.style, {
      background: "#ffffff",
      border: "1px solid #f0b0c8",
      borderRadius: "10px",
      padding: "10px 14px",
      color: "#2d2d2d",
      fontSize: "13px",
      boxShadow: "0 4px 20px rgba(196,48,96,.25)",
      cursor: "pointer",
      pointerEvents: "all",
      opacity: "0",
      transform: "translateX(110%)",
    ...
    name.textContent = "\u{1F4AC} " + senderName;
  ...
  var THEMES = {
    ...STATIC_THEMES,
    get custom() {
      return buildCustomTheme(state.STORE);
    }
  };
  ...
  function hasLocalSyncedPreferences() {
    return Object.keys(state.contactNotes || {}).length > 0 || state.pinnedContacts.size > 0 || state.mutedContacts.size > 0 || Object.keys(state.disappearingByConversation || {}).length > 0 || Object.keys(state.contactNotifyOverrides || {}).length > 0;
  }
  ...
  function getNotifyOverrideIcon(num) {
    ...
    if (mode === "always") return "\u{1F514}";
    if (mode === "never") return "\u{1F515}";
    return "";
  }
  async function syncServerBackedState() {
    ...
    if (!Object.keys(serverReactions).length) {
      const localServerReactions = Object.entries(state.msgReactions || {}).filter(([key, value]) => isServerBackedMessageKey(key) && value && typeof value === "object" && Object.keys(value).length);
  ...
  function resolveBcWhisperContent(data) {
    ...
    if (rawContent) {
      ...
      for (const entry of dictionary) {
        ...
        formatted = formatted.replace(new RegExp(`\\{${escapedTag}\\}`, "g"), text).replace(new RegExp(`%${escapedTag}%`, "g"), text).replace(new RegExp(`\\b${escapedTag}\\b`, "g"), text);
      }
      if (formatted && formatted !== rawContent) return stripBcFormattingTrailer(formatted);
    }
    const preferredTag = ["Message", "Content", "ChatMessage", "Text", "AutomaticSentence"].map((tag) => dictionary.find((entry) => String(entry?.Tag ?? "").toLowerCase() === tag.toLowerCase())).find(Boolean);
    if (preferredTag?.Text) return stripBcFormattingTrailer(preferredTag.Text);
    const longestText = dictionary.map((entry) => String(entry?.Text ?? "").trim()).filter(Boolean).sort((a, b) => b.length - a.length)[0];
  ...
  function getMessagePreviewText(content, deleted = false) {
    ...
    if (parsed.oneTime) return "\u{1F510} One-time message";
    if (parsed.spoiler) return "\u{1F648} Spoiler message";
    return parsed.text || "";
  }
  ...
  function closeMsgSearch() {
    ...
    if (bar) {
      ...
      const inp = bar.querySelector(".bcm-msgsearch-input");
      if (inp) inp.value = "";
    }
  ...
  function closeTmplPanel() {
    state.tmplPanelEl?.remove();
    state.tmplPanelEl = null;
  }
  function toggleTemplatePanel(inputEl) {
    if (state.tmplPanelEl) {
      closeTmplPanel();
      return;
    }
    const rect = inputEl.getBoundingClientRect();
    state.tmplPanelEl = el("div", {
      cls: "bcm-qr-panel",
      style: { left: rect.left + "px", bottom: window.innerHeight - rect.top + 6 + "px", maxWidth: rect.width + "px" }
    });
    if (!state.messageTemplates.length) {
      ...
      for (const t of state.messageTemplates) {
        ...
        state.tmplPanelEl.appendChild(el(
          "div",
          { cls: "bcm-qr-item", title: t.text, onclick: () => {
            const start = inputEl.selectionStart ?? inputEl.value.length;
            const end = inputEl.selectionEnd ?? inputEl.value.length;
  ...
  function createContactAvatar(memberNum, name, online = false, cls = "bcm-avatar") {
    ...
    const avatar = el(
      "div",
      { cls: `${cls}${availCls ? " " + availCls : ""}`, style },
      ...avatarUrl ? [el("img", {
        cls: "bcm-avatar-img",
        src: avatarUrl,
  ...
  function buildDialog() {
    ...
    state.dialogEl = el(
      ...
      el(
        "div",
        { cls: "bcm-titlebar" },
        el("div", { cls: "bcm-dtitle" }, "\u{1F4AC} BC Messenger"),
        el("div", { cls: "bcm-gear", title: "Settings", onclick: (e) => {
          e.stopPropagation();
          openSettingsDialog();
        } }, "\u2699"),
        el("div", { cls: "bcm-dclose", onclick: closeDialog }, "\xD7")
      ),
      ...
      el(
        ...
        el(
          ...
          el(
            "div",
            { cls: "bcm-icon-strip" },
            el("button", { cls: "bcm-strip-btn bcm-friends-tab", title: "Friends", onclick: () => toggleFriendsPanel() }, "\u{1F464}"),
            el("button", { cls: "bcm-strip-btn bcm-roomusers-tab", title: "Room users", onclick: () => toggleRoomUsersPanel() }, "\u{1F465}"),
            el("button", { cls: "bcm-strip-btn bcm-lobby-tab", title: "Lobby", onclick: () => toggleLobbyPanel() }, "\u{1F3E0}"),
            el("button", { cls: "bcm-strip-btn bcm-starred-tab", title: "Starred", onclick: () => toggleStarredPanel() }, "\u2B50"),
            el("button", { cls: "bcm-strip-btn bcm-collections-tab", title: "Collections", onclick: () => toggleCollectionsPanel() }, "\u{1F4C1}"),
            el("button", { cls: "bcm-strip-btn bcm-state.unread-tab", title: "Unread", onclick: () => toggleUnreadPanel() }, "\u{1F4EC}"),
            el("button", { cls: "bcm-strip-btn", title: "Saved messages", onclick: () => openSavedMessages() }, "\u{1F516}"),
            el("div", { cls: "bcm-strip-sep" }),
            el("button", { cls: "bcm-strip-btn", title: "Import friends", onclick: () => openBCImportDialog() }, "\u{1F4E5}"),
            el("button", { cls: "bcm-strip-btn", title: "Import room", onclick: () => openRoomImportDialog() }, "\u{1F4CD}"),
            el("button", { cls: "bcm-strip-btn", title: "Broadcast", onclick: () => openBroadcastDialog() }, "\u{1F4E3}")
          ),
          el("div", { cls: "bcm-clist" }),
          el(
            "button",
            { cls: "bcm-addbtn", onclick: promptAddContact },
            "+",
            el("span", { cls: "bcm-addbtn-label" }, " New message")
          ),
          el(
            "button",
            { cls: "bcm-addbtn", onclick: promptCreateGroup, style: { marginTop: "4px" } },
            "\u{1F465}",
            el("span", { cls: "bcm-addbtn-label" }, " New group")
          ),
          el(
            "button",
            { cls: "bcm-addbtn", onclick: openJoinGroupDialog, style: { marginTop: "4px" } },
            "\u{1F517}",
          ...
          el("button", {
            ...
            onclick: () => {
              ...
              if (expanded) {
                const collapse = () => {
                  sidebar?.classList.remove("bcm-sidebar-expanded");
        ...
        el(
          ...
          el(
            ...
            el(
              ...
              el(
                ...
                el("button", { cls: "bcm-header-btn bcm-edit-contact-btn", title: "Edit contact (name/notes/avatar)", disabled: true, onclick: () => state.selectedContact && openEditContactDialog(state.selectedContact) }, "\u{1F5BC}")
              ),
              el("div", { cls: "bcm-msghead-status" }, "")
            ),
            el("button", { cls: "bcm-header-btn bcm-disappear-btn", title: "Disappearing messages", style: { display: "none" }, onclick: cycleDisappearingSetting }, "\u{1F550}"),
            ...
            el("button", { cls: "bcm-search-btn", title: "Search messages", onclick: toggleMsgSearch }, "\u{1F50D}"),
            el(
              "div",
              { cls: "bcm-overflow-wrap" },
              el("button", {
                cls: "bcm-header-btn bcm-overflow-btn",
                title: "More actions",
                onclick: (e) => {
                  e.stopPropagation();
              ...
              el(
                ...
                el("button", { cls: "bcm-overflow-item bcm-media-btn", style: { display: "none" }, onclick: () => {
                  toggleHeaderOverflow();
                  toggleMediaPanel();
                } }, "\u{1F5BC} Media"),
                el("button", { cls: "bcm-overflow-item", title: "Scheduled messages", onclick: () => {
                  toggleHeaderOverflow();
                  openScheduledMessagesPanel();
                } }, "\u23F0 Scheduled"),
                el("button", {
                  cls: "bcm-overflow-item bcm-export-json-btn",
                  onclick: () => {
                    toggleHeaderOverflow();
                    if (state.selectedContact) exportConversation(state.selectedContact);
                    else if (state.selectedGroup) exportConversation(null);
                  }
                }, "\u2B07 Export"),
                el("button", {
                  cls: "bcm-overflow-item bcm-export-html-btn",
                  onclick: () => {
                    toggleHeaderOverflow();
                    if (state.selectedContact) exportConversationHtml(state.selectedContact);
                    else if (state.selectedGroup) exportConversationHtml(null);
                  }
                }, "\u{1F5CE} Export HTML"),
                el("button", {
                  cls: "bcm-overflow-item bcm-join-room-btn",
                  style: { display: "none" },
                  onclick: () => {
                    toggleHeaderOverflow();
                    const r = state.contactMeta[state.selectedContact]?.room;
                    if (r) joinRoom(r);
                  }
                }, "\u{1F6AA} Join their room"),
                el("button", {
                  cls: "bcm-overflow-item bcm-invite-room-btn",
                  style: { display: "none" },
                  onclick: () => {
                    toggleHeaderOverflow();
                    if (state.selectedContact) sendRoomInvite(state.selectedContact);
                  }
                }, "\u{1F3E0} Invite to my room")
              )
            )
          ),
          el(
            "div",
            { cls: "bcm-msgsearch-bar" },
            el("input", {
              cls: "bcm-msgsearch-input",
              type: "text",
              placeholder: "Search messages\u2026",
              oninput: (e) => runMsgSearch(e.target.value),
              onkeydown: (e) => {
                if (e.key === "Enter") stepMsgSearch(e.shiftKey ? -1 : 1);
                e.stopPropagation();
              }
            }),
          ...
          el(
            "div",
            { cls: "bcm-pin-banner" },
            el("span", { cls: "bcm-pin-banner-icon" }, "\u{1F4CC}"),
            el("span", { cls: "bcm-pin-banner-text" }, ""),
            el("button", { cls: "bcm-pin-all-btn", title: "Show all pins", onclick: (e) => {
              e.stopPropagation();
              openPinnedMessagesPanel();
            } }, "All pins"),
            el("button", { cls: "bcm-pin-banner-close", title: "Unpin", onclick: (e) => {
              e.stopPropagation();
              unpinLatestMessage();
            } }, "\u2715")
          ),
          ...
          el(
            ...
            el(
              ...
              el("button", {
                ...
                onclick: (e) => {
                  e.stopPropagation();
                  const inp = state.dialogEl?.querySelector(".bcm-input");
                  if (inp) toggleStickerPanel(inp);
                }
              }, "\u{1F5BC}"),
              el("button", {
                cls: "bcm-emoji-btn",
            ...
            el(
              ...
              el("button", {
                ...
                onclick: (e) => {
                  e.stopPropagation();
                  const inp = state.dialogEl?.querySelector(".bcm-input");
                  if (inp) toggleTemplatePanel(inp);
                }
              }, "\u{1F4DD}"),
              el("button", {
                cls: "bcm-compose-flag-btn bcm-spoiler-btn",
                title: "Spoiler message (hidden until clicked)",
                onclick: (e) => {
                  e.stopPropagation();
                  state.composeSpoiler = !state.composeSpoiler;
                  syncComposeModeButtons();
                }
              }, "\u{1F648}"),
              el("button", {
                cls: "bcm-compose-flag-btn bcm-onetime-btn",
                title: "One-time message (deleted after viewing)",
                onclick: (e) => {
                  e.stopPropagation();
                  state.composeOneTime = !state.composeOneTime;
                  syncComposeModeButtons();
                }
              }, "1\xD7"),
              el("button", {
                cls: "bcm-compose-flag-btn",
                title: "Create a poll (question + options)",
                onclick: (e) => {
                  e.stopPropagation();
              ...
              el("button", {
                cls: "bcm-compose-flag-btn bcm-activity-btn",
                title: "In-game action (only visible when contact is in your room)",
                style: { display: "none" },
                onclick: (e) => {
                  e.stopPropagation();
                  toggleActivityPanel();
  ...
  function onGlobalKeydown(e) {
    ...
    if (e.key === "Escape") {
      if (document.querySelector(".bcm-ctx-menu")) {
        closeContextMenu();
        return;
      }
  ...
  function toggle(label, initialValue, onChange, tooltip = "") {
    ...
    const row = el(
      "div",
      { cls: "bcm-settings-row", title: tooltip || "" },
      el("span", { cls: "bcm-settings-label", title: tooltip || "" }, label),
      el(
        "label",
        { cls: "bcm-toggle", title: tooltip || "" },
        inp,
        el("span", { cls: "bcm-toggle-track" }),
        el("span", { cls: "bcm-toggle-thumb" })
      )
    );
    inp.title = tooltip || "";
    return row;
  ...
  async function wipeAllLocalAccountData() {
    ...
    state.mutedContacts = /* @__PURE__ */ new Set();
    state.pinnedContacts = /* @__PURE__ */ new Set();
    ...
    state.msgReactions = {};
    state.starredMessages = /* @__PURE__ */ new Set();
  ...
  function openSettingsDialog() {
    ...
    state.settingsEl.appendChild(
      el(
        "div",
        { cls: "bcm-settings-titlebar" },
        el("span", { cls: "bcm-settings-title" }, "\u2699 BC Messenger Settings"),
        el("button", { cls: "bcm-settings-close", onclick: closeSettingsDialog }, "\xD7")
      )
    );
    const THEME_META = {
    ...
    for (const [key, meta] of Object.entries(THEME_META)) {
      const swatch = el(
        "div",
        {
          cls: `bcm-theme-swatch${state.currentTheme === key ? " active" : ""}`,
          "data-theme": key,
          title: `Switch theme to ${meta.label}`,
          onclick: () => {
            if (key !== "custom") applyTheme(key);
          }
        },
    ...
    for (const [size, label] of fontSizes) {
      fontRow.appendChild(el("button", {
        cls: `bcm-fontsize-btn${state.fontSize === size ? " active" : ""}`,
        "data-size": size,
        title: `Use ${size} font size`,
    ...
    function renderTmplSettings(container) {
      ...
      state.messageTemplates.forEach((t, i) => {
        list.appendChild(el(
          ...
          el(
            "span",
            { cls: "bcm-qr-settings-text", title: t.text },
            el("strong", {}, t.name + ": "),
            t.text
          ),
          el("button", { cls: "bcm-qr-settings-del", title: "Delete", onclick: () => {
      ...
      try {
        ...
        list.forEach((num) => {
          container.appendChild(el(
            ...
            el("button", { cls: "bcm-settings-btn", style: { flex: "0 0 auto", padding: "4px 10px", fontSize: "11px" }, onclick: async () => {
              try {
                await removeTrustedContact(num);
                await refreshTrustedContactsCache();
                showNote(`Removed #${num} from trusted`, false);
                openSettingsDialog();
              } catch (e) {
                showNote(`Failed: ${e.message}`, true);
              }
            } }, "Remove")
          ));
        });
      } catch {
        container.innerHTML = el("div", { style: { fontSize: "12px", color: "var(--bcm-text-muted)" } }, "Unable to load").outerHTML;
      }
    }
      ...
      try {
        ...
        archived.forEach((f) => {
          ...
          container.appendChild(el(
            ...
            el("button", { cls: "bcm-settings-btn", style: { flex: "0 0 auto", padding: "4px 10px", fontSize: "11px" }, onclick: async () => {
              try {
                await setConversationFolder(num, "inbox");
                showNote(`Unarchived conversation with #${num}`, false);
                openSettingsDialog();
              } catch (e) {
                showNote(`Failed: ${e.message}`, true);
              }
    ...
    async function renderMessageRequests() {
      ...
      try {
        ...
        requests.forEach((r) => {
          ...
          container.appendChild(el(
            "div",
            { style: { display: "flex", flexDirection: "column", gap: "4px", padding: "6px 0", borderBottom: "1px solid var(--bcm-border)" } },
            ...
            el(
              ...
              el("button", { cls: "bcm-settings-btn", style: { padding: "4px 10px", fontSize: "11px" }, onclick: async () => {
                try {
                  await acceptMessageRequest(r.id);
                  showNote("Request accepted", false);
              ...
              el("button", { cls: "bcm-settings-btn", style: { padding: "4px 10px", fontSize: "11px" }, onclick: async () => {
                try {
                  ...
                  showNote("Request declined", false);
                  renderMessageRequests();
                } catch (e) {
                  showNote(`Failed: ${e.message}`, true);
                }
    ...
    const TABS = [
      { id: "appearance", label: "\u{1F3A8} Appearance" },
      { id: "notifs", label: "\u{1F514} Notifications" },
      { id: "profile", label: "\u{1F464} Profile" },
      { id: "privacy", label: "\u{1F512} Privacy" },
      { id: "connection", label: "\u{1F50C} Connection" },
    ...
    panes.notifs.append(
      ...
      toggle("Toast pop-ups", state.toastsEnabled, (v) => {
        state.toastsEnabled = v;
        GM_setValue(state.STORE + "toasts", v);
      }, "Show in-page popup notifications for new messages."),
      toggle("Notification sound", state.soundEnabled, (v) => {
        state.soundEnabled = v;
        GM_setValue(state.STORE + "sound", v);
      ...
      el(
        ...
        el("input", {
          type: "number",
          min: String(MIN_AWAY_TIMEOUT_MINS),
          max: String(MAX_AWAY_TIMEOUT_MINS),
      ...
      el(
        "div",
        { cls: "bcm-settings-row", style: { flexDirection: "column", alignItems: "stretch", gap: "6px" } },
        ...
        el("textarea", {
          ...
          oninput: (e) => {
            state.afkMessage = e.target.value;
            GM_setValue(state.STORE + "afkMessage", state.afkMessage);
          }
        })
      ),
      ...
      qrContainer,
      el("hr", { cls: "bcm-settings-divider" }),
      el("div", { cls: "bcm-settings-section" }, "\u{1F4DD} Message Templates"),
      el("div", { cls: "bcm-settings-hint" }, "Reusable messages with a name. Click \u{1F4DD} in the compose bar to insert."),
      tmplContainer,
      el("hr", { cls: "bcm-settings-divider" }),
      el("div", { cls: "bcm-settings-section" }, "\u{1F514} Keyword Alerts"),
      el("div", { cls: "bcm-settings-hint" }, "Get alerted when these words appear in any message, even when muted."),
      el(
        "div",
        { cls: "bcm-kw-list" },
        ...state.keywordAlerts.map(
          (kw, i) => el(
            "div",
            { cls: "bcm-kw-chip" },
            el("span", {}, kw),
      ...
      el(
        ...
        (() => {
          const kwInp = el("input", { cls: "bcm-modal-input", type: "text", placeholder: "Add keyword\u2026", style: { flex: "1" } });
          kwInp.id = "bcm-kw-input";
          return kwInp;
        })(),
        el("button", { cls: "bcm-btn", onclick: () => {
          const inp = document.getElementById("bcm-kw-input");
          const val = inp?.value.trim();
          if (!val || state.keywordAlerts.includes(val)) {
            if (inp) inp.value = "";
            return;
          }
        ...
        ...state.autoResponderRules.map(
          (rule, i) => el(
            ...
            el(
              "span",
              { cls: "bcm-rule-label" },
              rule.matchType === "any" ? "\u{1F4AC} Any message" : `\u{1F50D} "${rule.keyword}"`,
              " \u2192 ",
      ...
      el("button", { cls: "bcm-btn", style: { marginTop: "6px", alignSelf: "flex-start", fontSize: "11px", padding: "4px 10px" }, onclick: async () => {
        const matchPick = await openSelect("When should this rule fire?", [
          { label: "\u{1F50D} Keyword match", value: "keyword" },
          { label: "\u{1F4AC} Any incoming message", value: "any" }
        ]);
        if (!matchPick) return;
        let keyword = "";
    ...
    panes.profile.append(
      ...
      el(
        ...
        el(
          ...
          el("option", { value: "online" }, "\u{1F7E2} Online"),
          el("option", { value: "away" }, "\u{1F7E1} Away"),
          el("option", { value: "dnd" }, "\u{1F534} Do Not Disturb"),
      ...
      el(
        ...
        el("input", {
          ...
          maxlength: String(MAX_STATUS_LENGTH),
      ...
      el(
        ...
        el("input", {
          ...
          onchange: (e) => {
            state.dndStartTime = e.target.value;
            GM_setValue(state.STORE + "dndStart", state.dndStartTime);
            if (state.loggedIn) setAvailability(state.availabilityState, state.dndStartTime, state.dndEndTime).catch(() => {
            });
          }
        })
      ...
      el(
        ...
        el("input", {
          type: "time",
          value: state.dndEndTime,
          style: { padding: "6px 8px", border: "1px solid var(--bcm-border)", borderRadius: "6px", fontSize: "12px", background: "var(--bcm-bg-input)", color: "var(--bcm-text)", width: "140px" },
          onchange: (e) => {
            state.dndEndTime = e.target.value;
            GM_setValue(state.STORE + "dndEnd", state.dndEndTime);
            if (state.loggedIn) setAvailability(state.availabilityState, state.dndStartTime, state.dndEndTime).catch(() => {
            });
          }
        })
      ...
      el(
        ...
        el("input", {
          ...
          oninput: (e) => {
            state.profilePronouns = String(e.target.value ?? "").slice(0, 40);
          }
        })
      ),
      el(
        ...
        el(
          "select",
          {
            style: { padding: "6px 8px", border: "1px solid var(--bcm-border)", borderRadius: "6px", fontSize: "12px", background: "var(--bcm-bg-input)", color: "var(--bcm-text)", width: "180px" },
            onchange: (e) => {
              state.profileTimezone = e.target.value;
            }
          },
          el("option", { value: "", selected: !state.profileTimezone }, "\u2014 Select \u2014"),
          ...[
            { offset: "UTC-12", label: "UTC-12 (Baker Island)" },
            { offset: "UTC-11", label: "UTC-11 (Midway)" },
            { offset: "UTC-10", label: "UTC-10 (Hawaii)" },
            { offset: "UTC-9", label: "UTC-9 (Alaska)" },
            { offset: "UTC-8", label: "UTC-8 (Pacific, NA)" },
            { offset: "UTC-7", label: "UTC-7 (Mountain, NA)" },
            { offset: "UTC-6", label: "UTC-6 (Central, NA)" },
            { offset: "UTC-5", label: "UTC-5 (Eastern, NA)" },
            { offset: "UTC-4", label: "UTC-4 (Atlantic)" },
            { offset: "UTC-3", label: "UTC-3 (Brazil, ARG)" },
            { offset: "UTC-2", label: "UTC-2 (S. Georgia)" },
            { offset: "UTC-1", label: "UTC-1 (Azores)" },
            { offset: "UTC+0", label: "UTC+0 (London, GMT)" },
            { offset: "UTC+1", label: "UTC+1 (Berlin, CET)" },
            { offset: "UTC+2", label: "UTC+2 (Athens, EET)" },
            { offset: "UTC+3", label: "UTC+3 (Moscow, MSK)" },
            { offset: "UTC+4", label: "UTC+4 (Dubai)" },
            { offset: "UTC+5", label: "UTC+5 (Karachi)" },
            { offset: "UTC+6", label: "UTC+6 (Dhaka)" },
            { offset: "UTC+7", label: "UTC+7 (Bangkok)" },
            { offset: "UTC+8", label: "UTC+8 (Shanghai, SGT)" },
            { offset: "UTC+9", label: "UTC+9 (Tokyo, JST)" },
            { offset: "UTC+10", label: "UTC+10 (Sydney)" },
            { offset: "UTC+11", label: "UTC+11 (Solomon)" },
            { offset: "UTC+12", label: "UTC+12 (Auckland)" }
          ].map((tz) => {
            const opts = { value: tz.offset };
            if (state.profileTimezone === tz.offset) opts.selected = true;
            return el("option", opts, tz.label);
          })
        )
      ),
      el(
      ...
      el(
        ...
        el(
          "select",
          { style: { padding: "6px 8px", border: "1px solid var(--bcm-border)", borderRadius: "6px", fontSize: "12px", background: "var(--bcm-bg-input)", color: "var(--bcm-text)", width: "140px" }, onchange: (e) => {
            state.profilePrivacy.bio = e.target.value;
          } },
      ...
      el(
        ...
        el(
          "select",
          { style: { padding: "6px 8px", border: "1px solid var(--bcm-border)", borderRadius: "6px", fontSize: "12px", background: "var(--bcm-bg-input)", color: "var(--bcm-text)", width: "140px" }, onchange: (e) => {
            state.profilePrivacy.pronouns = e.target.value;
          } },
      ...
      el(
        ...
        el(
          "select",
          { style: { padding: "6px 8px", border: "1px solid var(--bcm-border)", borderRadius: "6px", fontSize: "12px", background: "var(--bcm-bg-input)", color: "var(--bcm-text)", width: "140px" }, onchange: (e) => {
            state.profilePrivacy.timezone = e.target.value;
          } },
      ...
      el(
        ...
        el("button", { cls: "bcm-settings-btn", title: "Save your profile to the server", onclick: async () => {
          ...
          try {
            ...
            if (r?.success) showNote("Profile saved", false);
            else throw new Error(r?.error || "Failed");
          } catch (e) {
            showNote(`Profile save failed: ${e.message}`, true);
          }
        } }, "Save profile"),
        el("button", { cls: "bcm-settings-btn", title: "Load your profile from the server", onclick: async () => {
          ...
          try {
            ...
            openSettingsDialog();
            showNote("Profile loaded", false);
          } catch (e) {
            showNote(`Profile load failed: ${e.message}`, true);
          }
        } }, "Load from server")
      ),
      el("hr", { cls: "bcm-settings-divider" }),
      ...
      el(
        ...
        el("button", { cls: "bcm-settings-btn", onclick: async () => {
          ...
          try {
            await addTrustedContact(num);
            await refreshTrustedContactsCache();
            showNote(`Added #${num} to trusted contacts`, false);
            openSettingsDialog();
          } catch (e) {
            showNote(`Failed: ${e.message}`, true);
          }
    ...
    panes.connection.append(
      ...
      el(
        ...
        el("input", {
          ...
          maxlength: String(MAX_DISCORD_WEBHOOK_LENGTH),
          placeholder: "https://discord.com/api/webhooks/...",
          style: { padding: "6px 8px", border: "1px solid var(--bcm-border)", borderRadius: "6px", fontSize: "12px", background: "var(--bcm-bg-input)", color: "var(--bcm-text)" },
          oninput: (e) => {
            state.discordWebhookUrl = String(e.target.value ?? "").slice(0, MAX_DISCORD_WEBHOOK_LENGTH);
            GM_setValue(state.STORE + "discordWebhook", state.discordWebhookUrl);
            if (state.loggedIn) register().catch(() => {
            });
          }
        })
      ...
      el(
        ...
        el("button", { cls: "bcm-settings-btn", title: "Delete local chat history and local contacts only", onclick: async () => {
          ...
          if (ml) {
            ml.innerHTML = "";
            ml.appendChild(el("div", { cls: "bcm-empty" }, "History cleared"));
          }
          closeSettingsDialog();
        } }, "Clear history"),
        el("button", { cls: "bcm-settings-btn danger", title: "Delete all local BCM data and remove your account data from the server database", onclick: async () => {
          ...
          try {
            ...
            await openAlert(`Server data deletion failed: ${e.message}`);
            return;
          }
        ...
        el("button", { cls: "bcm-settings-btn", title: "Fetch and merge recent server history into local storage", onclick: async () => {
          try {
            const count = await syncHistoryFromServer(500);
            refreshContactList();
            showNote(`Synced ${count} message(s)`, false);
          } catch (e) {
            showNote(`Sync failed: ${e.message}`, true);
          }
        } }, "Sync history"),
  ...
  function closeSettingsDialog() {
    state.settingsEl?.classList.remove("bcm-open");
  }
  function exportSettingsBackup() {
    ...
    const keysToExport = [
      ...
      "starred",
      "disappearing",
      "readReceiptsDisabledConversations",
      "pinnedMessages",
      "scheduled",
      "availability",
      "dndStart",
    ...
    a.href = url;
    a.download = `bcm-backup-${state.memberNumber || "unknown"}-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.json`;
    a.click();
...
Current settings will be overwritten.`)) return;
  ...
  async function resolveUnknownNames() {
    const unknowns = state.allContacts.filter((c) => isMemberNumberLikeName(c.memberName, c.memberNum) && !state.resolvedAttempts.has(c.memberNum)).slice(0, 5);
  ...
  async function resolveUnknownFriends() {
    ...
    const unknowns = friendNums.filter((num) => isMemberNumberLikeName(state.contactMeta[num]?.name, num) && !state.resolvedAttempts.has(num)).slice(0, 20);
    if (!unknowns.length) return;
  ...
  async function renderContactList(filter) {
    ...
    const uniqueFolders = [...new Set(
      Object.values(state.conversationFolders).map((f) => f.folder).filter((f) => f && f !== "archive" && f !== "inbox")
    )];
    ...
    if (uniqueFolders.length > 0 || hasSnoozed) {
      const tabRow = el(
        ...
        { cls: "bcm-folder-tabs" },
        el("div", {
          cls: `bcm-folder-tab${state.activeFolderFilter === "all" ? " active" : ""}`,
          onclick: () => {
            state.activeFolderFilter = "all";
        ...
        ...uniqueFolders.map(
          (f) => el("div", {
            cls: `bcm-folder-tab${state.activeFolderFilter === f ? " active" : ""}`,
            onclick: () => {
              state.activeFolderFilter = f;
              renderContactList(filter);
            }
          }, `\u{1F4C1} ${f}`)
        ),
        hasSnoozed ? el("div", {
          cls: `bcm-folder-tab${state.activeFolderFilter === "snoozed" ? " active" : ""}`,
          onclick: () => {
            state.activeFolderFilter = "snoozed";
            renderContactList(filter);
          }
        }, "\u{1F634} Snoozed") : null
      );
      list.appendChild(tabRow);
    } else if (state.activeFolderFilter !== "all") {
      state.activeFolderFilter = "all";
    }
    const snoozedTimes = Object.values(state.conversationFolders).map((f) => f.snoozedUntil).filter((t) => t && t > now);
    ...
    const shownGroups = Object.values(state.groups).filter(
      (g) => !q || g.name.toLowerCase().includes(q) || String(g.id).includes(q)
    ...
    for (const g of shownGroups) {
      ...
      list.appendChild(
        el(
          ...
          el("div", { cls: "bcm-group-icon", style: { background: g.avatarColor || "#8b7fa8" } }, "\u{1F465}"),
          el(
            "div",
            { cls: "bcm-cinfo" },
    ...
    if (onlineContacts.length > 0) {
      ...
      for (const c of sortedOnline) {
        ...
        const name = getSafeDisplayName(state.contactMeta[num]?.name, num, "") || getSafeDisplayName(c.memberName, num, "") || `Member #${num}`;
        const online = true;
        ...
        const pinIcon = isPinned(num) ? [el("span", { cls: "bcm-pin-icon", title: "Pinned" }, "\u{1F4CC}")] : [];
        const muteIcon = isMuted(num) ? [el("span", { cls: "bcm-mute-icon", title: "Muted" }, "\u{1F507}")] : [];
        ...
        const statusParts = [];
        if (avClass === "away") statusParts.push("\u{1F7E1} Away");
        else if (avClass === "dnd") statusParts.push("\u{1F534} DND");
        const cstatus = state.contactMeta[num]?.status;
        const room = state.contactMeta[num]?.room;
        if (cstatus) statusParts.push(cstatus);
        else if (room) statusParts.push(`\u{1F4CD} ${room}`);
        const statusLine = statusParts.length ? [el("div", { cls: "bcm-cstatus" }, statusParts.join(" \xB7 "))] : [];
          ...
          el(
            "div",
            {
              ...
              oncontextmenu: (e) => {
                e.preventDefault();
                e.stopPropagation();
                openContactContextMenu(num, e.clientX, e.clientY);
              }
            },
            ...
            el(
              "div",
              { cls: "bcm-contact-actions" },
              el("button", { cls: "bcm-cact-btn", title: "Send message", onclick: (e) => {
                e.stopPropagation();
                selectContact(num, name);
              } }, "\u{1F4AC}"),
              el("button", { cls: "bcm-cact-btn", title: "View profile", onclick: (e) => {
                e.stopPropagation();
                openProfileCard(num);
              } }, "\u{1F464}")
            )
    ...
    if (!renderedAny && !renderedSearchResults) {
      list.appendChild(el(
        "div",
        { style: { padding: "14px", color: "#3a2850", fontSize: "12px", textAlign: "center" } },
    ...
    function rebuildTagChips() {
      ...
      for (const tag of editTags) {
        tagChipsEl.appendChild(el(
          ...
          tag,
          el("button", {
            style: { background: "none", border: "none", cursor: "pointer", color: "var(--bcm-text-muted)", padding: "0 0 0 2px", fontSize: "11px", lineHeight: "1" },
            onclick: () => {
  ...
  async function openProfileCard(contactNum) {
    ...
    const AVAIL_LABEL = { online: "\u{1F7E2} Online", away: "\u{1F7E1} Away", dnd: "\u{1F534} Do Not Disturb", invisible: "\u26AB Offline", offline: "\u26AB Offline" };
    ...
    const lastSeenText = !isOnline && lastSeen ? `Last seen ${new Date(lastSeen).toLocaleString()}` : null;
    const header = el(
      "div",
      { style: { display: "flex", gap: "14px", alignItems: "flex-start", width: "100%" } },
      ...
      el(
        ...
        el("span", { style: {
          ...
          fontWeight: "600",
          color: availColor,
          background: "var(--bcm-bg-side)",
          border: `1px solid ${availColor}`,
          borderRadius: "10px",
          padding: "2px 8px"
        } }, availLabel)
      )
    );
    const statusRow = status ? el("div", { style: { fontSize: "12px", fontStyle: "italic", color: "var(--bcm-text-muted)", borderLeft: "2px solid var(--bcm-border)", paddingLeft: "8px" } }, `"${status}"`) : null;
    const lastSeenRow = lastSeenText ? el("div", { style: { fontSize: "11px", color: "var(--bcm-text-muted)" } }, lastSeenText) : null;
    const detailItems = [
      ...pronouns ? [["Pronouns", pronouns]] : [],
      ...timezone ? [["Timezone", timezone]] : []
    ];
    const detailGrid = detailItems.length ? el(
      "div",
      { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 12px", width: "100%" } },
      ...detailItems.map(
        ([label, val]) => el(
          "div",
          { style: { fontSize: "11px" } },
    ...
    const badgeRow = badges.length ? el(
      "div",
      { style: { display: "flex", gap: "6px", flexWrap: "wrap" } },
      ...badges.map((b) => el("span", { style: { background: "var(--bcm-accent)", color: "#fff", padding: "2px 8px", borderRadius: "10px", fontSize: "11px" } }, b))
    ) : null;
    const trustedChip = profile?.isTrusted ? el("div", { style: { fontSize: "11px", color: "#c43060", fontWeight: "600" } }, "\u2B50 Trusted contact") : null;
    const heatBuckets = getHeatBuckets(contactNum);
    const heatMax = Math.max(1, ...heatBuckets);
    const heatRow = Math.max(...heatBuckets) > 0 ? el(
      "div",
      { cls: "bcm-heat-row", title: "Typical online hours (your local time)" },
      el("div", { cls: "bcm-heat-label" }, "\u{1F550} Typically online"),
      el(
        "div",
        { cls: "bcm-heat-bars" },
        ...heatBuckets.map((v, h) => {
          const pct = Math.round(v / heatMax * 100);
          const bar = el("div", { cls: "bcm-heat-bar", title: `${String(h).padStart(2, "0")}:00 \u2014 seen ${v}\xD7` });
  ...
  function openContactContextMenu(contactNum, x, y) {
    ...
    const pinEmoji = isPinned(contactNum) ? "\u{1F4CC} " : "\u{1F4CC} ";
    const muteEmoji = isMuted(contactNum) ? "\u{1F514} " : "\u{1F507} ";
    state.ctxMenuEl = el(
      "div",
      { cls: "bcm-ctx-menu" },
      el("div", { cls: "bcm-ctx-item", onclick: () => {
        togglePin(contactNum);
        closeContextMenu();
      } }, pinEmoji + pinLabel),
      el("div", { cls: "bcm-ctx-item", onclick: () => {
        toggleMute(contactNum);
        closeContextMenu();
      } }, muteEmoji + muteLabel),
      el("div", { cls: "bcm-ctx-item", onclick: () => {
      ...
      el("div", { cls: "bcm-ctx-item", onclick: async () => {
        closeContextMenu();
        await openEditContactDialog(contactNum);
      } }, "\u270F Edit"),
      el("div", { cls: "bcm-ctx-item", onclick: async () => {
        closeContextMenu();
        await openProfileCard(contactNum);
      ...
      el("div", { cls: "bcm-ctx-item", onclick: async () => {
        ...
        try {
          ...
          showNote(`Archive failed: ${e.message}`, true);
        }
      } }, "\u{1F4E6} Archive"),
      el("div", { cls: "bcm-ctx-item", onclick: async () => {
        closeContextMenu();
      ...
      } }, `\u{1F4C1} ${state.conversationFolders[contactNum]?.folder && state.conversationFolders[contactNum].folder !== "archive" && state.conversationFolders[contactNum].folder !== "inbox" ? `Folder: ${state.conversationFolders[contactNum].folder}` : "Move to folder"}`),
      el("div", { cls: "bcm-ctx-item", onclick: async () => {
        closeContextMenu();
        ...
        try {
          await snoozeConversation(contactNum, mins * 60 * 1e3);
          if (mins > 0) {
            state.conversationFolders[contactNum] = { ...state.conversationFolders[contactNum] || {}, snoozedUntil: Date.now() + mins * 60 * 1e3 };
          ...
          showNote(`Snooze failed: ${e.message}`, true);
        }
      } }, state.conversationFolders[contactNum]?.snoozedUntil > Date.now() ? "\u{1F514} Clear snooze" : "\u{1F515} Snooze"),
      state.trustedContacts.has(contactNum) ? el("div", { cls: "bcm-ctx-item", onclick: async () => {
        closeContextMenu();
        try {
          await removeTrustedContact(contactNum);
          await refreshTrustedContactsCache();
          showNote(`Removed #${contactNum} from trusted`, false);
        } catch (e) {
          showNote(`Failed: ${e.message}`, true);
        }
      } }, "\u2B50 Untrust") : el("div", { cls: "bcm-ctx-item", onclick: async () => {
        closeContextMenu();
        try {
          await addTrustedContact(contactNum);
          await refreshTrustedContactsCache();
          showNote(`Added #${contactNum} to trusted`, false);
        } catch (e) {
          showNote(`Failed: ${e.message}`, true);
        }
      } }, "\u2B50 Trust"),
      el("div", { cls: "bcm-ctx-item", onclick: () => {
        closeContextMenu();
        sendFriendInvite(contactNum);
      } }, "\u{1F44B} Invite as BC friend"),
      ...unsafeWindow.ChatRoomData?.Name ? [el("div", { cls: "bcm-ctx-item", onclick: () => {
        closeContextMenu();
        sendRoomInvite(contactNum);
      } }, "\u{1F3E0} Invite to my room")] : [],
      ...
      el("div", { cls: "bcm-ctx-item", onclick: () => {
        ...
        refreshContactList();
      } }, state.contactNotifyOverrides[String(contactNum)] === "always" ? "\u{1F514} \u2713 Always notify" : "\u{1F514} Always notify"),
      el("div", { cls: "bcm-ctx-item", onclick: () => {
        closeContextMenu();
        state.contactNotifyOverrides[String(contactNum)] = "never";
        persistNotifyOverrides();
        refreshContactList();
      } }, state.contactNotifyOverrides[String(contactNum)] === "never" ? "\u{1F515} \u2713 Never notify" : "\u{1F515} Never notify"),
      el("div", { cls: "bcm-ctx-item", onclick: () => {
        closeContextMenu();
        delete state.contactNotifyOverrides[String(contactNum)];
        persistNotifyOverrides();
        refreshContactList();
      } }, !state.contactNotifyOverrides[String(contactNum)] ? "\u{1F514} \u2713 Default" : "\u{1F514} Default"),
      isBlockedMember(contactNum) ? el("div", { cls: "bcm-ctx-item", onclick: async () => {
        closeContextMenu();
      ...
      } }, "\u2705 Unblock") : el("div", { cls: "bcm-ctx-item", onclick: async () => {
        closeContextMenu();
...
Describe the issue (optional):`);
      ...
      el("div", {
        cls: "bcm-ctx-item bcm-ctx-danger",
        onclick: async () => {
          closeContextMenu();
          ...
          if (state.selectedContact === contactNum) {
            ...
            if (ml) {
              ml.innerHTML = "";
              ml.appendChild(el("div", { cls: "bcm-empty" }, "Conversation deleted"));
            }
            const hName = state.dialogEl?.querySelector(".bcm-msghead-name");
    ...
    setTimeout(() => {
      const outside = (e) => {
        if (!state.ctxMenuEl?.contains(e.target)) {
          closeContextMenu();
  ...
  async function editMessageByKey(msg, reactionKey) {
    ...
    if (!bubble) {
      const parsed2 = parseQuotePayload(msg.content ?? "");
      const next = await openPrompt("Edit message:", parsed2.text ?? "");
      if (next == null) return;
      const trimmed = String(next).trim();
      if (!trimmed) return;
      const newContent = encodeQuotePayload(trimmed, parsed2.quote);
  ...
  function openMessageContextMenu(msg, reactionKey, x, y) {
    ...
    const pinLabel = canPin && isPinnedMessage(reactionKey) ? "\u{1F4CC} Unpin" : "\u{1F4CC} Pin";
    const editedItems = msg?.edited && msg?.serverId ? [
      el("div", { cls: "bcm-ctx-item", onclick: async () => {
        ...
        try {
          ...
          if (!revs.length) {
            showNote("No edit history found", false);
            return;
          }
          openEditHistoryModal(revs);
        } catch {
          showNote("Could not load edit history", true);
        }
      } }, "\u{1F4DC} Edit history")
    ] : [];
    const hasParent = !!msg?.parentMessageRef;
    state.ctxMenuEl = el(
      "div",
      { cls: "bcm-ctx-menu" },
      hasParent ? el("div", { cls: "bcm-ctx-item", onclick: () => {
        scrollToMessageByRef(msg.parentMessageRef);
        closeContextMenu();
      } }, "\u{1F53C} Jump to parent") : null,
      el("div", { cls: "bcm-ctx-item", onclick: () => {
        closeContextMenu();
      ...
      el("div", { cls: "bcm-ctx-item", onclick: () => {
        setQuote(msg, reactionKey);
        closeContextMenu();
        state.dialogEl?.querySelector(".bcm-input")?.focus();
      } }, "\u21A9 Reply"),
      el("div", { cls: "bcm-ctx-item", onclick: () => {
        toggleStarByKey(reactionKey);
        closeContextMenu();
      } }, starLabel),
      el("div", { cls: "bcm-ctx-item", onclick: async () => {
        closeContextMenu();
      ...
      el("div", { cls: "bcm-ctx-item", onclick: async () => {
        ...
        const r = {
          ...
          remindAt: Date.now() + pick.value * 60 * 1e3
        };
      ...
      ...canPin ? [el("div", { cls: "bcm-ctx-item", onclick: () => {
        togglePinMessage(msg, reactionKey);
        closeContextMenu();
      } }, pinLabel)] : [],
      ...editedItems,
      ...mine ? [
        el("div", { cls: "bcm-ctx-item", onclick: () => {
          editMessageByKey(msg, reactionKey);
          closeContextMenu();
        } }, "\u270F Edit message"),
        el("div", { cls: "bcm-ctx-item bcm-ctx-danger", onclick: () => {
          deleteMessageByKey(msg, reactionKey);
          closeContextMenu();
        } }, "\u{1F5D1} Delete message")
      ] : []
    );
  ...
  function toggleCurrentConversationReadReceipts() {
    ...
    updateReadReceiptHeaderButton();
    showNote(
      canSendReadReceiptsForConversation(key) ? "Read receipts enabled for this conversation" : "Read receipts disabled for this conversation",
      false
    );
  }
  ...
  function getDisappearConfigForConversation(key) {
    ...
    if (raw && typeof raw === "object") {
      const ttlMs2 = Number(raw.ttlMs ?? 0);
      const enabledAt = Number(raw.enabledAt ?? 0);
      return {
        ttlMs: Number.isFinite(ttlMs2) && ttlMs2 >= 0 ? ttlMs2 : 0,
  ...
  function updateDisappearingHeaderButton() {
    ...
    btn.textContent = cfg.ttlMs > 0 ? `\u{1F550}${option.label}` : "\u{1F550}";
  ...
  function updateReadReceiptHeaderButton() {
    ...
    if (!state.sendReadReceipts) {
      ...
      btn.title = enabled ? "Read receipts are enabled for this conversation" : "Read receipts are disabled for this conversation";
    }
  ...
  async function cycleDisappearingSetting() {
    ...
    if (next.value > 0 && state.selectedContact && !state.selectedGroup && !state.contactMeta[state.selectedContact]?.hasBCM) {
      const choice = await openModal({
        ...
        body: el(
          "div",
          { style: { display: "flex", flexDirection: "column", gap: "10px", fontSize: "12px", color: "var(--bcm-text)" } },
        ...
        buttons: [
          { label: "\u{1F4E8} Invite them", primary: true, value: "invite" },
  ...
  async function openManageGroupDialog(groupId) {
    ...
    for (const m of normalized) {
      ...
      const row = el(
        "div",
        { cls: "bcm-member-row" },
        el("span", { cls: "bcm-member-row-name" }, `${m.role === "admin" ? "\u{1F451} " : "\u{1F464} "}${name} (#${num})`)
      );
      if (!isMe) {
        if (m.role !== "admin") {
          row.appendChild(el("button", { cls: "bcm-modal-btn", style: { fontSize: "11px", padding: "2px 7px" }, onclick: async () => {
            try {
              ...
              await openAlert(`Promote failed: ${ex.message}`);
            }
          } }, "\u25B2 Promote"));
        } else {
          row.appendChild(el("button", { cls: "bcm-modal-btn", style: { fontSize: "11px", padding: "2px 7px" }, onclick: async () => {
            try {
              ...
              await openAlert(`Demote failed: ${ex.message}`);
            }
          } }, "\u25BC Demote"));
        }
        row.appendChild(el("button", { cls: "bcm-modal-btn danger", style: { fontSize: "11px", padding: "2px 7px" }, onclick: async () => {
          ...
          try {
            ...
            await openAlert(`Kick failed: ${ex.message}`);
          }
        } }, "\u2715 Kick"));
      }
    ...
    const actionsEl = el(
      ...
      el("button", { cls: "bcm-modal-btn", onclick: async () => {
        ...
        try {
          ...
          await openAlert(`Rename failed: ${ex.message}`);
        }
      } }, "\u270F Rename"),
      el("button", { cls: "bcm-modal-btn primary", onclick: async () => {
        ...
        try {
          ...
          await openAlert(`Add member failed: ${ex.message}`);
        }
      } }, "+ Add member"),
      el("button", { cls: "bcm-modal-btn", onclick: async () => {
        try {
          ...
          const inviteBody = el(
            "div",
            { style: { display: "flex", flexDirection: "column", gap: "10px", maxWidth: "360px" } },
          ...
          if (clicked === "copy") {
            try {
              await navigator.clipboard.writeText(r.inviteText || r.token);
              showNote("Invite copied to clipboard", false);
            } catch {
              showNote("Copy failed \u2014 select and copy manually", true);
            }
          }
        } catch (ex) {
          await openAlert(`Join link failed: ${ex.message}`);
        }
      } }, "\u{1F517} Join link"),
    ...
    if (Array.isArray(joinRequests) && joinRequests.length) {
      joinRequestsEl.appendChild(el(
        "div",
        { style: { fontSize: "11px", fontWeight: "bold", color: "var(--bcm-accent)", margin: "4px 0 2px" } },
        `\u{1F4E5} Join requests (${joinRequests.length})`
      ));
        ...
        row.appendChild(el("button", {
          ...
          onclick: async () => {
            try {
              ...
              await openAlert(`Accept failed: ${ex.message}`);
            }
          }
        }, "\u2713 Accept"));
        row.appendChild(el("button", {
          cls: "bcm-modal-btn danger",
          style: { fontSize: "11px", padding: "2px 7px" },
          onclick: async () => {
            try {
              ...
              await openAlert(`Decline failed: ${ex.message}`);
            }
          }
        }, "\u2715 Decline"));
  ...
  async function getMemberSearchCandidates() {
    ...
    const add = (num, name, username = "", aliases = []) => {
      ...
      const aliasSet = /* @__PURE__ */ new Set([...prev.aliases || [], ...aliases.map((v) => String(v || "").trim()).filter(Boolean)]);
      map.set(n, {
        memberNum: n,
  ...
  function getFriendListCandidates() {
    return getBCFriendEntries().map((entry) => {
  ...
  function buildMemberSearchLabel(candidate) {
    ...
    return username && username.toLowerCase() !== name.toLowerCase() ? `${name} \xB7 @${username} \xB7 #${num}` : `${name} \xB7 #${num}`;
  }
  ...
  async function findMemberCandidates(query, limit = 8) {
    const ranked = (await getMemberSearchCandidates()).map((candidate) => ({ ...candidate, score: scoreMemberCandidate(candidate, query) })).filter((candidate) => candidate.score >= 0).sort((a, b) => b.score - a.score || String(a.name || "").localeCompare(String(b.name || "")));
  ...
  async function openAmbiguousMemberAlert(candidates, token = "") {
    return new Promise((resolve) => {
      ...
      card.appendChild(
        el(
          "div",
          { cls: "bcm-modal-body", style: { display: "flex", flexDirection: "column", gap: "6px", textAlign: "left" } },
  ...
  async function searchLocalMessages(query, limit = 24) {
    ...
    for (const msg of messages) {
      ...
      const senderLabel = groupId ? getDisplayNameForMember(msg.senderNum, msg.senderName || `Member #${msg.senderNum}`) : msg.fromUs ? state.memberName ?? `Member #${state.memberNumber}` : getDisplayNameForMember(partnerNum, contactNameByNum[partnerNum] || `Member #${partnerNum}`);
      const scopeLabel = groupId ? groupNameById[groupId] || state.groups[groupId]?.name || `Group #${groupId}` : contactNameByNum[partnerNum] || state.contactMeta[partnerNum]?.name || `Member #${partnerNum}`;
  ...
  async function renderSidebarMessageSearchResults(list, query, runId) {
    ...
    for (const result of results) {
      list.appendChild(el(
        ...
        el("div", { cls: "bcm-search-result-meta" }, `${result.scopeLabel} \xB7 ${result.senderLabel} \xB7 ${new Date(result.sentAt || Date.now()).toLocaleString()}`),
  ...
  function isPinned(num) {
    return state.pinnedContacts.has(Number(num));
  }
  function isMuted(num) {
    return state.mutedContacts.has(Number(num));
  }
  function togglePin(num) {
    num = Number(num);
    if (state.pinnedContacts.has(num)) state.pinnedContacts.delete(num);
    else state.pinnedContacts.add(num);
    scheduleSyncedPreferencesSave();
    refreshContactList();
  }
  function toggleMute(num) {
    num = Number(num);
    if (state.mutedContacts.has(num)) state.mutedContacts.delete(num);
    else state.mutedContacts.add(num);
    scheduleSyncedPreferencesSave();
    refreshContactList();
  }
  ...
  function togglePinMessage(msg, reactionKey) {
    ...
    if (idx >= 0) {
      ...
      const senderLabel = isGroupMsg ? getDisplayNameForMember(msg.senderNum, msg.senderName || `Member #${msg.senderNum}`) : msg.fromUs ? state.memberName ?? `Member #${state.memberNumber}` : state.contactMeta[msg.partnerNum]?.name || `Member #${msg.partnerNum}`;
  ...
  function openPinnedMessagesPanel() {
    ...
    return new Promise((resolve) => {
      ...
      card.appendChild(el("div", { cls: "bcm-onetime-title" }, `\u{1F4CC} Pinned messages (${pins.length})`));
      ...
      if (!pins.length) {
        ...
        [...pins].reverse().forEach((p) => {
          ...
          info.appendChild(el("div", { cls: "bcm-scheduled-item-meta" }, `${p.senderLabel} \xB7 ${p.sentAt ? new Date(p.sentAt).toLocaleString() : ""}`));
  ...
  function removeScheduledMessage(id) {
    if (state.scheduledTimers[id]) {
      clearTimeout(state.scheduledTimers[id]);
      delete state.scheduledTimers[id];
    }
      ...
      if (item.groupId) {
        ...
        if (result?.success) {
          ...
          let msg = {
            ...
            senderNum: state.memberNumber,
            senderName: state.memberName || `Member #${state.memberNumber}`,
          ...
          try {
            msg = await saveGroupMessage(item.groupId, state.memberNumber, state.memberName || `Member #${state.memberNumber}`, item.content, sentAt, true, null, status, false, result?.groupMessageRef ?? null, result?.receipt ?? null);
          } catch {
          }
          if (result?.senderMessageId) updateMessageServerId(msg, result.senderMessageId).catch(() => {
          });
          if (state.selectedGroup === item.groupId) {
            appendBubble(msg);
            scrollMsgs();
          }
        } else {
        ...
        if (result?.success) {
          ...
          try {
            msg = await saveMessage(cn, item.content, sentAt, true, null, "sent");
          } catch {
          }
          if (result?.messageId) updateMessageServerId(msg, result.messageId).catch(() => {
          });
          if (state.selectedContact === cn && !state.selectedGroup) {
            appendBubble(msg);
            scrollMsgs();
          }
        } else {
  ...
  function showUpdateBanner(latestVersion) {
    ...
    bar.innerHTML = "";
    bar.style.display = "flex";
    bar.appendChild(document.createTextNode("\u{1F195} Update available \u2014 v" + latestVersion + "  "));
    ...
    const dismiss = el("button", { cls: "bcm-update-dismiss", title: "Dismiss" }, "\xD7");
  ...
  async function openBCImportDialog() {
    ...
    if (bcFriends.length === 0) {
      await openAlert("All your BC friends are already imported as contacts!");
      return;
    }
    ...
    await new Promise((resolve) => {
      ...
      const updateCount = () => {
        countLbl.textContent = `${selected.size} selected`;
      };
      const rows = bcFriends.map((f) => {
        ...
        chk.addEventListener("change", () => {
          if (chk.checked) selected.add(num);
          else selected.delete(num);
          updateCount();
        });
        return el(
          "label",
          { style: { display: "flex", alignItems: "center", gap: "8px", padding: "5px 10px", cursor: "pointer", borderBottom: "1px solid var(--bcm-border)" } },
          chk,
          el("span", { style: { fontSize: "12px", color: "var(--bcm-text)" } }, f.name || `Member #${num}`),
          f.room ? el("span", { style: { fontSize: "10px", color: "var(--bcm-text-muted)", marginLeft: "auto" } }, `\u{1F4CD} ${f.room}`) : null
        );
      }).filter(Boolean);
      const card = el(
        "div",
        { cls: "bcm-onetime-card", style: { minWidth: "min(340px,94vw)", maxWidth: "min(500px,96vw)" } },
        el("div", { cls: "bcm-onetime-title" }, "\u{1F4E5} Import BC Friends"),
        ...
        el(
          ...
          el("button", { cls: "bcm-modal-btn primary", type: "button", onclick: async () => {
            ...
            for (const f of bcFriends) {
              if (selected.has(f.memberNum)) {
                await upsertContact(f.memberNum, f.name || `Member #${f.memberNum}`);
                imported++;
              }
            }
            await refreshContactList();
  ...
  async function openBroadcastDialog() {
    const contacts = Object.entries(state.contactMeta).filter(([num]) => Number(num) !== state.memberNumber).map(([num, m]) => ({ num: Number(num), name: m.name || `Member #${num}` })).sort((a, b) => a.name.localeCompare(b.name));
    if (contacts.length === 0) {
      await openAlert("No contacts to broadcast to.");
      return;
    }
    ...
    await new Promise((resolve) => {
      ...
      const rows = contacts.map((c) => {
        const chk = el("input", { type: "checkbox" });
        chk.addEventListener("change", () => {
          if (chk.checked) selected.add(c.num);
          else selected.delete(c.num);
          updateCount();
        });
        return el(
          "label",
      ...
      const card = el(
        "div",
        { cls: "bcm-onetime-card", style: { minWidth: "min(360px,94vw)", maxWidth: "min(520px,96vw)" } },
        el("div", { cls: "bcm-onetime-title" }, "\u{1F4E3} Broadcast Message"),
        el("div", { cls: "bcm-modal-body" }, "Send the same message to multiple contacts."),
        textarea,
        el(
          "div",
          { style: { display: "flex", justifyContent: "space-between", alignItems: "center", margin: "8px 0 4px" } },
        ...
        el(
          ...
          el("button", { cls: "bcm-modal-btn primary", type: "button", onclick: async () => {
            ...
            if (!text) {
              showNote("Enter a message first", true);
              return;
            }
            if (selected.size === 0) {
              showNote("Select at least one recipient", true);
              return;
            }
            finish();
            let sent = 0;
  ...
  async function openPollCreationDialog() {
      ...
      el(
        ...
        el(
          "select",
          {
            ...
            style: { flex: "1" },
            onchange: (e) => {
              pollDuration = parseInt(e.target.value, 10) || 0;
            }
          },
    ...
    if (state.selectedGroup) {
      try {
        ...
        await openAlert(`Poll failed: ${e.message}`);
      }
    } else {
      try {
  ...
  function openScheduleSendDialog() {
    ...
    const defaultDate = new Date(now.getTime() + 60 * 60 * 1e3);
    ...
    return new Promise((resolve) => {
      ...
      card.appendChild(el(
        "div",
        { cls: "bcm-modal-body", style: { display: "flex", flexDirection: "column", gap: "8px" } },
      ...
      card.appendChild(el(
        ...
        el("button", { cls: "bcm-modal-btn", type: "button", style: { background: "var(--bcm-accent)", color: "#fff", border: "none" }, onclick: () => {
          ...
          if (!dt) {
            showNote("Please pick a date and time", true);
            return;
          }
          const sendAt = new Date(dt).getTime();
          if (!sendAt || isNaN(sendAt)) {
            showNote("Invalid date/time", true);
            return;
          }
          if (sendAt <= Date.now()) {
            showNote("Scheduled time must be in the future", true);
            return;
          }
    ...
    return new Promise((resolve) => {
      ...
      const renderList = () => {
        ...
        [...state.scheduledMessages].sort((a, b) => a.sendAt - b.sendAt).forEach((item) => {
          ...
          const convName = item.groupId ? state.groups[item.groupId]?.name || `Group #${item.groupId}` : state.contactMeta[item.contactNum]?.name || `Member #${item.contactNum}`;
          const info = el("div", { style: { flex: "1", minWidth: 0 } });
          info.appendChild(el("div", { cls: "bcm-scheduled-item-meta" }, `${convName} \xB7 ${new Date(item.sendAt).toLocaleString()}`));
      ...
      if (preUnread > 0 && list) {
        ...
        if (firstUnreadMsg) {
          const targetBubble = list.querySelector(
            firstUnreadMsg.serverId ? `[data-sid="${firstUnreadMsg.serverId}"]` : `[data-local-id="${firstUnreadMsg.id}"]`
          );
          if (targetBubble) {
  ...
  async function selectGroup(groupId) {
    ...
    if (!leaveBtn && msgHead) {
      leaveBtn = el("button", { cls: "bcm-header-btn bcm-leave-group-btn", title: "Leave group", onclick: () => confirmLeaveGroup(groupId) }, "\u{1F6AA}");
    ...
    const unreadIncomingIds = msgs.filter((m) => Number(m.senderNum) !== Number(state.memberNumber) && m.serverId).map((m) => Number(m.serverId)).filter(Number.isFinite);
    ...
    syncGroupHistoryFromServer(groupId, 250).then(async (added) => {
  ...
  function renderConversationSlice(msgs, startIdx) {
    ...
    if (startIdx > 0) {
      ...
      const btn = el("button", { cls: "bcm-load-more", onclick: async () => {
        const allMsgs = state.selectedGroup ? await getGroupMessages(state.selectedGroup) : await getMessages(state.selectedContact);
  ...
  function formatGroupReceiptLabel(msg) {
    ...
    if (read > 0) return `Seen ${read}/${total} \xB7 Delivered ${delivered}/${total}`;
  ...
  async function openSeenByModal(msg) {
    ...
    openModal({ title: "\u{1F441} Seen by", body, buttons: [{ label: "Close", value: "close" }] });
    let receipts = [];
    try {
      const r = await apiGetGroupReceipts(msg.groupMessageRef);
      receipts = r?.receipts ?? [];
    } catch {
    }
    const receiptMap = {};
    ...
    members.sort((a, b) => {
      const rank = (m) => {
        const r = receiptMap[m.member_number];
        return r?.read_at ? 0 : r?.delivered ? 1 : 2;
      };
    ...
    const rows = members.map((m) => {
      ...
      const timeStr = isRead ? new Date(Number(rec.read_at)).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "";
      ...
      return el(
        ...
        el(
          ...
          el(
            "div",
            { cls: `bcm-seenby-status${isRead ? " bcm-seenby-read" : ""}` },
            el("span", { cls: tickCls }, isRead || isDelivered ? " \u2713\u2713" : " \u2713"),
            ` ${statusLabel}`,
            timeStr ? el("span", { cls: "bcm-seenby-time" }, ` \xB7 ${timeStr}`) : null
          )
        )
      );
    });
  ...
  function openOneTimeViewer(parsed) {
    ...
    state.oneTimeViewerEl = el(
      "div",
      { cls: "bcm-onetime-overlay" },
      el(
        "div",
        { cls: "bcm-onetime-card" },
  ...
  function openEditHistoryModal(revisions) {
    const overlay = el(
      "div",
      { cls: "bcm-onetime-overlay", onclick: (e) => {
        if (e.target === overlay) overlay.remove();
      } },
      el(
        "div",
        { cls: "bcm-onetime-card", style: { minWidth: "min(400px, 94vw)", maxWidth: "min(600px, 96vw)" } },
        el("div", { cls: "bcm-onetime-title" }, `\u{1F4DC} Edit history (${revisions.length} revision${revisions.length !== 1 ? "s" : ""})`),
        el(
          "div",
          { style: { display: "flex", flexDirection: "column", gap: "8px", overflowY: "auto", maxHeight: "60vh" } },
          ...revisions.map((rev, i) => {
            const when = new Date(rev.revised_at).toLocaleString();
            return el(
              "div",
              { style: { borderLeft: "3px solid var(--bcm-accent)", paddingLeft: "8px", fontSize: "12px" } },
              el("div", { style: { color: "var(--bcm-text-muted)", fontSize: "11px", marginBottom: "2px" } }, `v${i + 1} \xB7 ${when}`),
  ...
  function appendBubble(msg, prevSentAt = null) {
    ...
    if (_rawContent.startsWith(E2E_V2G_PREFIX) || _rawContent.startsWith(E2E_V2_PREFIX) || _rawContent.startsWith(E2E_PREFIX)) {
      _rawContent = "[\u{1F512} Encrypted \u2014 could not decrypt]";
    }
    ...
    const hasParent = !!msg.parentMessageRef;
    const quoteBlock = parsed.quote ? [el(
      "div",
      { cls: `bcm-quote-inline${hasParent ? " bcm-quote-clickable" : ""}`, title: hasParent ? "Click to view parent message" : "", onclick: hasParent ? () => {
        scrollToMessageByRef(msg.parentMessageRef);
      } : null },
    ...
    const renderedContent = parsed.oneTime && !msg.deleted ? [el(
      "div",
      { cls: "bcm-msg-content" },
      el("div", {}, "\u{1F510} One-time message"),
    ...
    )] : parsed.spoiler && !msg.deleted ? [el("div", { cls: "bcm-msg-content" }, createSpoilerNode(contentNodes))] : contentText?.startsWith("\u{1F3E0}ROOMINVITE:") && !msg.deleted ? (() => {
      const roomName = contentText.slice("\u{1F3E0}ROOMINVITE:".length);
      return [el(
        "div",
        { cls: "bcm-msg-content" },
        el("button", {
          cls: "bcm-room-invite-card",
          onclick: () => unsafeWindow.ServerSend?.("ChatRoomJoin", { Name: roomName })
        }, `\u{1F3E0} Join room: ${roomName}`)
      )];
    })() : [el("div", { cls: "bcm-msg-content" }, ...contentNodes)];
    const bubble = el(
      "div",
      { cls: `bcm-bubble ${isMine ? "sent" : "recv"}${hasAnyReactions ? " has-reaction" : ""}${starred ? " is-starred" : ""}` },
      ...senderLabel,
      ...quoteBlock,
      ...
      el("button", {
        cls: "bcm-react-btn",
        title: reaction ? `Your reaction: ${reaction}. Click to change` : "React",
        "aria-label": reaction ? `Change reaction: ${REACTION_LABELS[reaction] || reaction}` : "React to message",
        onclick: (e) => {
          e.stopPropagation();
          openReactionPanel(e.currentTarget, reactionKey);
        }
      }, reaction || "\u{1F60A}"),
      el(
        "div",
        ...
        ...parsed.spoiler ? [el("span", { cls: "bcm-disappear-mark", title: "Spoiler message" }, "\u{1F648}")] : [],
        ...parsed.oneTime ? [el("span", { cls: "bcm-disappear-mark", title: "One-time message" }, "\u{1F510}")] : []
      ),
    ...
    bubble.addEventListener("click", (e) => {
      ...
      if (!key) return;
      if (state.selectedMsgs.has(key)) {
        state.selectedMsgs.delete(key);
        bubble.classList.remove("bcm-selected");
      } else {
        state.selectedMsgs.add(key);
        bubble.classList.add("bcm-selected");
      }
      updateSelectionBar();
    });
    ...
    if (lpUrl && !IMAGE_URL_RE.test(lpUrl) && !VIDEO_URL_RE.test(lpUrl) && !msg.deleted) {
      fetchLinkPreview(lpUrl).then((preview) => {
        ...
        const card = el(
          "div",
          { cls: "bcm-link-preview", onclick: () => window.open(preview.url || lpUrl, "_blank") },
          ...
          el(
            ...
            el("div", { cls: "bcm-lp-domain" }, "\u{1F310} " + (preview.domain || ""))
          )
        );
        const btimeEl = bubble.querySelector(".bcm-btime");
        if (btimeEl) bubble.insertBefore(card, btimeEl);
  ...
  var IMAGE_URL_RE = /\.(png|jpe?g|gif|webp|bmp|svg)$/i;
  var VIDEO_URL_RE = /\.(mp4|webm|ogg|mov|m4v)$/i;
  var YOUTUBE_VIDEO_ID_LENGTH = 11;
  var YOUTUBE_VIDEO_ID_RE = new RegExp(`^[a-zA-Z0-9_-]{${YOUTUBE_VIDEO_ID_LENGTH}}$`);
  var YOUTUBE_IFRAME_ALLOW = "accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share";
  var REACTION_EMOJIS = ["\u{1F44D}", "\u2764\uFE0F", "\u{1F602}", "\u{1F62E}", "\u{1F622}", "\u{1F44E}", "\u{1F525}", "\u{1F44F}"];
  var REACTION_LABELS = { "\u{1F44D}": "thumbs up", "\u2764\uFE0F": "heart", "\u{1F602}": "laugh", "\u{1F62E}": "surprised", "\u{1F622}": "sad", "\u{1F44E}": "thumbs down", "\u{1F525}": "fire", "\u{1F44F}": "clap" };
  var EMOJI_CATALOG = [
    "\u{1F600}",
    "\u{1F601}",
    "\u{1F602}",
  ...
  function renderPollWidget(poll, reactionKey, messageRef) {
    ...
    const myVote = cachedVote && cachedVote.myVote !== null && cachedVote.myVote !== void 0 ? cachedVote.myVote : null;
    ...
    (async () => {
      try {
        ...
        const voted = svMyVote !== null && svMyVote !== void 0;
  ...
  var REACTION_PANEL_OFFSET_X = 90;
  var REACTION_PANEL_OFFSET_Y = 44;
  var PANEL_EDGE_PADDING = 4;
  ...
  function createEmbedNode(url) {
    ...
    if (isVideoUrl(url)) {
      ...
      videoEl.addEventListener("error", () => {
        videoEl.replaceWith(el("div", { style: { fontSize: "12px", color: "var(--bcm-text-muted)", padding: "8px" } }, "\u{1F3AC} Video unavailable"));
      });
  ...
  function pushMarkdownNodes(nodes, text) {
    ...
    while ((m = MD.exec(text)) !== null) {
      ...
      if (m[1] !== void 0) {
        ...
        node.className = "bcm-inline-code";
  ...
  function renderMessageContent(text) {
    ...
    if (isHostedStickerUrl(trimmed)) {
      return [el(
        "div",
        { cls: "bcm-embed-wrap bcm-sticker-embed" },
        ...
        if (processedLines.length) {
          ...
          while ((m2 = URL_RE.exec(chunk)) !== null) {
            ...
            if (!safeUrl) {
              ...
              nodes.push(a);
              const emb = createEmbedNode(safeUrl);
              if (emb) nodes.push(emb);
            }
  ...
  function scrollToMessageByRef(ref) {
    ...
    if (bubble) {
      ...
      setTimeout(() => bubble.classList.remove("bcm-flash"), 2e3);
    } else {
  ...
  function updateBubbleReactionByKey(key) {
    ...
    if (reactBtn) {
      reactBtn.textContent = myEmoji || "\u{1F60A}";
    ...
    const chips = entries.map(([emoji, members]) => {
      ...
      return el("button", {
        ...
        onclick: (e) => {
          e.stopPropagation();
          setReactionByKey(key, mine ? "" : emoji);
        }
      }, emoji, count > 1 ? el("span", { cls: "bcm-reaction-count" }, String(count)) : null);
    });
    const addBtn = el("button", {
      cls: "bcm-reaction-add",
      type: "button",
      title: "Add a reaction",
      "aria-label": "Add a reaction",
      onclick: (e) => {
        e.stopPropagation();
        openReactionPanel(bubble.querySelector(".bcm-react-btn") || bubble, key);
      }
    }, "\uFF0B");
    const row = el("div", {
  ...
  function openReactedByModal(reactionKey) {
    ...
    if (!entries.length) {
      ...
      for (const [emoji, members] of entries) {
        body.appendChild(el("div", { cls: "bcm-react-who-head" }, `${emoji} \xD7 ${members.length}`));
    ...
    openModal({ title: "\u{1F60A} Reactions", body, buttons: [{ label: "Close", value: "close" }] });
  }
  ...
  function openReactionPanel(anchorEl, reactionKey) {
    if (!reactionKey) return;
    closeReactionPanel();
    const reactWith = async (emoji) => {
      await setReactionByKey(reactionKey, emoji);
      closeReactionPanel();
    };
    ...
    const searchEl = el("input", { cls: "bcm-emoji-search", type: "text", placeholder: "\u{1F50D} Search emojis\u2026" });
    searchEl.addEventListener("input", () => {
      const q = searchEl.value.trim().toLowerCase();
      populateGrid(q ? EMOJI_CATALOG.filter((e) => (REACTION_LABELS[e] || "").includes(q)) : EMOJI_CATALOG);
    });
    state.reactionPanelEl = el(
      "div",
      { cls: "bcm-react-panel bcm-react-panel-full" },
      el(
        "div",
        { cls: "bcm-react-quick" },
        ...REACTION_EMOJIS.map(
          (emoji) => el("button", { cls: `bcm-react-item${emoji === myEmoji ? " bcm-react-item-active" : ""}`, "aria-label": `React with ${REACTION_LABELS[emoji] || emoji}`, title: emoji === myEmoji ? "Your current reaction \u2014 click to remove" : "", onclick: () => reactWith(emoji) }, emoji)
        ),
    ...
    const maxLeft = containerRect ? Math.max(minLeft, containerRect.right - panelRect.width - PANEL_EDGE_PADDING) : Math.max(minLeft, window.innerWidth - panelRect.width - PANEL_EDGE_PADDING);
    const maxTop = containerRect ? Math.max(minTop, containerRect.bottom - panelRect.height - PANEL_EDGE_PADDING) : Math.max(minTop, window.innerHeight - panelRect.height - PANEL_EDGE_PADDING);
  ...
  function refreshHeaderOnlineStatus(num) {
    ...
    if (hStatus && hStatus.className !== "bcm-msghead-status bcm-typing") {
      ...
      if (!cur || cur === "Online" || cur === "Away" || cur === "Do Not Disturb" || cur === "Offline" || cur.startsWith("Last seen") || cur.startsWith("\u{1F4CD}")) {
  ...
  function showTypingIndicator(senderNum) {
    ...
    state.typingTimers[senderNum] = setTimeout(() => {
      if (hStatus && hStatus.dataset.prevText !== void 0) {
  ...
  async function exportConversation(contactNum) {
    ...
    if (!msgs.length) {
      showNote("No messages to export", false);
      return;
    }
    const name = isGroup ? state.groups[groupId]?.name ?? `Group #${groupId}` : state.contactMeta[contactNum]?.name ?? `Member #${contactNum}`;
    const lines = msgs.map((m) => {
      ...
      const sender = isGroup ? getDisplayNameForMember(m.senderNum, m.senderName || (m.fromUs ? state.memberName ?? `Member #${state.memberNumber}` : `Member #${m.senderNum}`)) : m.fromUs ? state.memberName ?? `Member #${state.memberNumber}` : name;
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `bcm-${name.replace(/[^a-z0-9]/gi, "_")}-${Date.now()}.txt`;
  ...
  function embedHtmlForUrl(url) {
    ...
    if (isVideoUrl(safeUrl))
      return `<div class="embed"><video src="${escapeHtml(safeUrl)}" controls preload="metadata" referrerpolicy="no-referrer" onerror="this.outerHTML='<div style=\\'font-size:12px;color:#888;padding:8px\\'>\u{1F3AC} Video unavailable</div>'"></video></div>`;
    return "";
  }
  ...
  async function exportConversationHtml(contactNum) {
    ...
    const bubbles = msgs.map((m) => {
      ...
      const sender = isGroup ? getDisplayNameForMember(m.senderNum, m.senderName || (isMine ? meName : `Member #${m.senderNum}`)) : isMine ? meName : name;
      return `
...
<body>
  ...
  var EMOJIS = "\u{1F60A} \u{1F602} \u{1F60D} \u{1F97A} \u{1F62D} \u{1F618} \u{1F60E} \u{1F525} \u2764\uFE0F \u{1F44D} \u{1F44E} \u{1F64F} \u{1F605} \u{1F60F} \u{1F914} \u{1F608} \u{1F480} \u{1F440} \u{1F389} \u{1F970} \u{1F621} \u{1F494} \u2728 \u{1F338} \u{1F495} \u{1F634} \u{1F923} \u{1F44B} \u{1FAE1} \u{1F917} \u{1F624} \u{1F4AC}".split(" ");
  ...
  async function openStickerPicker(inputEl) {
    ...
    const tabs = el(
      "div",
      { cls: "bcm-sticker-tabs" },
      el("button", { cls: "bcm-sticker-tab active", type: "button" }, "\u{1F5BC} Stickers"),
      el("button", { cls: "bcm-sticker-tab", type: "button" }, "\u{1F39E} GIFs")
    );
    const stickerGrid = el(
      "div",
      { cls: "bcm-sticker-grid" },
      el("div", { cls: "bcm-sticker-loading" }, "\u23F3 Loading stickers\u2026")
    );
    const stickerTab = el(
      "div",
      { cls: "bcm-sticker-tabpage" },
      el(
        "div",
        { cls: "bcm-sticker-toolbar" },
        el(
          "label",
          { cls: "bcm-sticker-upload", title: "Upload your own sticker (PNG, GIF, WebP, max 2MB)" },
          "\uFF0B Upload",
    ...
    const gifTab = el(
      "div",
      { cls: "bcm-sticker-tabpage", style: { display: "none" } },
      el(
        "div",
        { cls: "bcm-gif-searchrow" },
        el("input", { cls: "bcm-gif-search", type: "text", placeholder: "\u{1F50D} Search GIFs\u2026 (Enter to search)" })
      ),
      gifGrid,
      gifNote
    );
    ...
    gifTab.querySelector(".bcm-gif-search").addEventListener("keydown", (e) => {
      ...
      if (!q) return;
      gifGrid.appendChild(el("div", { cls: "bcm-sticker-loading" }, "\u{1F50E} Searching\u2026"));
      gifSearch(q, 24).then((res) => {
    ...
    tabs.querySelectorAll(".bcm-sticker-tab").forEach((btn2, i) => {
      btn2.addEventListener("click", () => {
        tabs.querySelectorAll(".bcm-sticker-tab").forEach((b) => b.classList.toggle("active", b === btn2));
  ...
  var ACTIVITY_ACTIONS = [
    { label: "\u{1F44B} Wave", verb: "waves at" },
    { label: "\u{1F917} Hug", verb: "hugs" },
    { label: "\u{1F618} Kiss", verb: "gives a kiss to" },
    { label: "\u{1F449} Poke", verb: "pokes" },
    { label: "\u{1F647} Bow", verb: "bows to" },
    { label: "\u{1F483} Dance", verb: "dances with" },
    { label: "\u270B High-five", verb: "high-fives" },
    { label: "\u{1F91D} Handshake", verb: "shakes hands with" }
  ];
  ...
  function toggleActivityPanel() {
    ...
    state.activityPanelEl = el(
      "div",
      { cls: "bcm-activity-panel" },
      ...ACTIVITY_ACTIONS.map(
        (a) => el("div", { cls: "bcm-activity-item", onclick: (e) => {
          e.stopPropagation();
  ...
  async function updateE2EIndicator(memberNum) {
    ...
    if (!memberNum || state.selectedGroup) {
      ind.textContent = "";
      ind.title = "";
      return;
    }
    ...
    if (r.status === "changed") {
      ...
      ind.textContent = verified ? "\u{1F512}\u2713" : "\u{1F512}";
  ...
  async function openE2EInfoDialog(memberNum) {
    ...
    if (r.status === "changed") {
      const sn2 = await safetyNumberFor(memberNum);
      const ok = confirm(
...
Old safety number (pinned): ${sn2 || "(none)"}
...
Click OK to ${verified ? "UN-mark" : "mark"} as verified, Cancel to close.`
  ...
  function onRoomSearchResult(data) {
    const list = Array.isArray(data) ? data : Array.isArray(data?.rooms) ? data.rooms : Array.isArray(data?.Results) ? data.Results : [];
  ...
  function renderFriendsPanel() {
    ...
    panel.appendChild(el(
      "div",
      { cls: "bcm-friends-header" },
      el("span", {}, "\u{1F465} Friends"),
    ...
    const friends = getFriendListCandidates().filter((f) => !q || String(f.name || "").toLowerCase().includes(q) || String(f.username || "").toLowerCase().includes(q) || String(f.memberNum || "").includes(q));
    ...
    for (const f of friends) {
      ...
      const secondary = room ? `\u{1F4CD} ${room}` : username && String(username).toLowerCase() !== String(name).toLowerCase() ? `@${username}` : state.contactMeta[f.memberNum]?.status || (online ? "Online" : "Offline");
      const subCls = room ? "bcm-friends-sub-room" : String(secondary).startsWith("@") ? "bcm-friends-sub-user" : "bcm-friends-sub";
      list.appendChild(el(
        "div",
        {
  ...
  async function renderStarredPanel() {
    ...
    for (const item of items.slice(0, MAX_STARRED_PANEL_ITEMS)) {
      ...
      list.appendChild(el(
        ...
        el("div", { cls: "bcm-starred-meta" }, `${scope} \xB7 ${new Date(item.sentAt || Date.now()).toLocaleString()}`),
  ...
  function toggleUnreadPanel() {
    ...
    if (state.unreadPanelOpen) {
      ...
      closeAllPanels();
      if (state.dialogEl) {
        const m = state.dialogEl.querySelector(".bcm-main");
  ...
  function renderUnreadPanel() {
    ...
    if (!panel) return;
    panel.innerHTML = "";
    panel.appendChild(el("div", { cls: "bcm-starred-header" }, "\u{1F4EC} Unread messages"));
    const rows = [];
    ...
    for (const r of rows) {
      const avatar = r.type === "dm" ? createContactAvatar(r.num, r.name, r.online) : el("div", { cls: "bcm-group-icon", style: { background: state.groups[r.gid]?.avatarColor || "#8b7fa8", flexShrink: "0" } }, "\u{1F465}");
      const row = el(
        "div",
        { cls: "bcm-state.unread-row", onclick: () => {
          closeAllPanels();
          const mainEl = state.dialogEl?.querySelector(".bcm-main");
          if (mainEl) mainEl.style.display = "";
  ...
  async function renderMediaPanel() {
    ...
    const header = el(
      "div",
      { cls: "bcm-starred-header" },
      el("button", { cls: "bcm-back-btn", onclick: () => {
        closeAllPanels();
        const m = state.dialogEl?.querySelector(".bcm-main");
        if (m) m.style.display = "";
      } }, "\u2190 Back"),
  ...
  function statRow(label, value) {
    return el(
      "div",
      { style: { display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: "1px solid var(--bcm-border)" } },
  ...
  async function openConversationStats(contactNum) {
    ...
    if (!msgs.length) {
      showNote("No messages yet", false);
      return;
    }
    ...
    const fmtDur = (ms) => ms < 6e4 ? "<1 min" : ms < 36e5 ? `${Math.round(ms / 6e4)} min` : `${Math.round(ms / 36e5)} hr`;
    const body = el(
      "div",
    ...
    openModal({ title: "\u{1F4CA} Conversation Stats", body, buttons: [{ label: "Close", value: "close" }] });
  }
  async function openRoomImportDialog() {
    const W = unsafeWindow;
    const chars = [...W.ChatRoomCharacter ?? [], ...W.Character ?? []];
    const inRoom = chars.map((c) => ({ num: parseInt(c.MemberNumber ?? c.ID ?? 0, 10), name: String(c.Name ?? "").trim() })).filter((c) => c.num && c.num !== state.memberNumber);
    ...
    if (!newOnes.length) {
      await openAlert(inRoom.length ? "Everyone in the room is already in your contacts." : "You're not in a room right now, or no other members are present.");
      return;
    }
    ...
    const updateCount = () => {
      countEl.textContent = `Import ${selected.size} contact${selected.size !== 1 ? "s" : ""}`;
    };
    updateCount();
    const rows = newOnes.map((c) => {
      ...
      chk.checked = true;
      chk.addEventListener("change", () => {
        chk.checked ? selected.add(c.num) : selected.delete(c.num);
        updateCount();
      });
      return el(
        "label",
        { style: { display: "flex", alignItems: "center", gap: "8px", padding: "5px 0", cursor: "pointer", fontSize: "12px" } },
        chk,
    ...
    openModal({ title: "\u{1F4CD} Import from Room", body, buttons: [{ label: "Cancel", value: "cancel" }] });
    setTimeout(() => {
  ...
  async function renderCollectionsPanel(detailName = null) {
    ...
    if (detailName !== null) {
      ...
      panel.appendChild(el("div", { cls: "bcm-starred-header" }, `\u{1F4C1} ${detailName}`));
      const list2 = el("div", { cls: "bcm-starred-list", style: { padding: "0", gap: "0" } });
      panel.appendChild(list2);
      try {
        const result = await getCollectionMessages(detailName);
        const msgs = Array.isArray(result?.messages) ? result.messages : [];
        if (!msgs.length) {
          list2.appendChild(el("div", { cls: "bcm-empty" }, "Collection is empty"));
          return;
        }
        const refMap = {};
          ...
          list2.appendChild(el(
            ...
            el("button", { cls: "bcm-coll-remove", title: "Remove from collection", onclick: async () => {
              try {
                ...
                showNote(`Remove failed: ${e.message}`, true);
              }
            } }, "\u2715 Remove")
          ));
        }
      } catch {
        list2.appendChild(el("div", { cls: "bcm-empty" }, "Could not load collection"));
      }
      return;
    }
    panel.appendChild(el("div", { cls: "bcm-starred-header" }, "\u{1F4C1} Message collections"));
    const list = el("div", { cls: "bcm-starred-list" });
    try {
      ...
      for (const col of cols) {
        list.appendChild(el(
          "div",
          {
            cls: "bcm-starred-item",
            style: { display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer" },
            onclick: () => renderCollectionsPanel(col.name)
          },
          el("span", { style: { fontWeight: "700", fontSize: "13px", color: "var(--bcm-text)" } }, `\u{1F4C1} ${col.name}`),
  ...
  function renderRoomUsersPanel() {
    ...
    panel.appendChild(el(
      "div",
      { cls: "bcm-roomusers-header" },
      el("div", { cls: "bcm-roomusers-title" }, "\u{1F465} " + roomName),
    ...
    for (const char of chars) {
      ...
      const whisperBtn = el("button", {
        cls: "bcm-roomuser-whisper",
        title: "Whisper in room",
        onclick: () => {
          closeAllPanels();
          const mainEl = state.dialogEl?.querySelector(".bcm-main");
          if (mainEl) mainEl.style.display = "";
      ...
      list.appendChild(
        el(
          "div",
          { cls: `bcm-roomuser${isMe ? " bcm-roomuser-me" : ""}` },
  ...
  function togglePinnedRoom(name) {
    ...
    if (i >= 0) state.pinnedRooms.splice(i, 1);
    else state.pinnedRooms.push(name);
  ...
  function renderLobbyPanel(rooms) {
    ...
    if (currentRoom) {
      panel.appendChild(el(
        "div",
        { cls: "bcm-lobby-current" },
        "\u{1F4CD}",
        el("div", { cls: "bcm-lobby-current-name" }, currentRoom)
      ));
    }
    const searchBox = el("input", {
      cls: "bcm-lobby-searchbox",
      type: "text",
      placeholder: "Filter rooms\u2026",
      value: state.lobbySearch
    });
    ...
    if (rooms === "noresponse") {
      panel.appendChild(el(
        "div",
        { cls: "bcm-empty", style: { flexDirection: "column", gap: "8px", fontSize: "12px" } },
        el("div", {}, "\u{1F3E0} Room list unavailable"),
        el(
          "div",
          { style: { color: "var(--bcm-text-muted)", fontSize: "11px", textAlign: "center", maxWidth: "200px" } },
  ...
  function renderLobbyFiltered(panel, rooms, currentRoom, searchBox, countEl) {
    ...
    function buildRoomRow(room, forPinnedSection) {
      ...
      const isPinned2 = state.pinnedRooms.includes(name);
      let cls = "bcm-room-item";
      if (isCurrent) cls += " bcm-room-current";
      else if (!forPinnedSection && hasFriend) cls += " bcm-room-friend";
      const lockIcon = isPrivate ? " \u{1F512}" : "";
      const label = (isCurrent ? "\u{1F4CD} " : hasFriend ? "\u2B50 " : "") + name + lockIcon;
      const capacityStr = limit ? `${count}/${limit}` : `${count}`;
      const meta = `${capacityStr} member${count !== 1 ? "s" : ""}${isPrivate ? " \xB7 Private" : ""}${desc ? " \xB7 " + desc.slice(0, 50) : ""}`;
      return el(
        "div",
        { cls },
        el(
        ...
        el("button", {
          cls: `bcm-room-pin-btn${isPinned2 ? " active" : ""}`,
          title: isPinned2 ? "Unpin room" : "Pin room",
          onclick: (e) => {
            e.stopPropagation();
            togglePinnedRoom(name);
          }
        }, isPinned2 ? "\u2605" : "\u2606"),
        isCurrent ? el("button", { cls: "bcm-room-join", disabled: "true" }, "Here") : state.pendingJoinTarget === name ? el("button", { cls: "bcm-room-join", disabled: "true" }, "Joining...") : el("button", { cls: "bcm-room-join", onclick: () => joinRoom(name) }, "Join")
      );
    }
  ...
  function joinRoom(name) {
    try {
      ...
      const inRoom = typeof W.ServerPlayerIsInChatRoom === "function" ? !!W.ServerPlayerIsInChatRoom() : W.CurrentScreen === "ChatRoom";
      if (inRoom) {
      ...
      const attemptJoin = () => {
        ...
        const stillInRoom = typeof W.ServerPlayerIsInChatRoom === "function" ? !!W.ServerPlayerIsInChatRoom() : false;
  ...
  async function doSend() {
    ...
    if (state.selectedContact && Number(state.selectedContact) === Number(state.memberNumber)) {
      ...
      try {
        msg2 = await saveMessage(state.memberNumber, state.memberNumber, content2, sentAt2, true, null, "delivered");
      } catch {
      }
      appendBubble(msg2);
      scrollMsgs();
    ...
    if (state.selectedContact && !state.selectedGroup) {
      ...
      if (usedBCMFeature && !state.contactMeta[state.selectedContact]?.hasBCM) {
        ...
        const choice = await openModal({
          ...
          buttons: [
            { label: "\u{1F4E8} Invite & Send", primary: true, value: "invite" },
    ...
    if (state.selectedGroup) {
      try {
        ...
        if (result?.success) {
          ...
          msg = updateGroupReceiptForMessage(msg, result?.receipt, result?.groupMessageRef ?? null);
          try {
            msg = await saveGroupMessage(state.selectedGroup, state.memberNumber, state.memberName || `Member #${state.memberNumber}`, outgoingContent, sentAt, true, null, status, false, result?.groupMessageRef ?? null, result?.receipt ?? null);
          } catch {
          }
    ...
    try {
      ...
      if (state.sendMode === "whisper") {
        ...
        const sentAt2 = Date.now();
        let msg2 = { partnerNum: rn, senderNum: state.memberNumber, content: outgoingContent, sentAt: sentAt2, fromUs: true, serverId: null, status: "delivered" };
        try {
          msg2 = await saveMessage(rn, state.memberNumber, outgoingContent, sentAt2, true, null, "delivered");
        } catch {
        }
        scheduleDisappearingForMessage(msg2);
        appendBubble(msg2);
        scrollMsgs();
        refreshContactList();
        sendToServer(rn, outgoingContent, false, state.currentParentMessageRef, true).then((r) => {
          if (r?.success && r.id) {
            updateMessageServerId(msg2, r.id).catch(() => {
            });
          }
        }).catch(() => {
        });
      ...
      try {
        msg = await saveMessage(rn, state.memberNumber, outgoingContent, sentAt, true, null, initialStatus);
      } catch {
      }
      scheduleDisappearingForMessage(msg);
      appendBubble(msg);
  ...
  function onInputChange(e) {
    ...
    if (state.selectedGroup) {
      ...
      if (atMatch) {
        ...
        const members = group?.members ? normalizeGroupMembers(group.members).map((m) => ({ num: Number(m.member_number), name: getDisplayNameForMember(Number(m.member_number), `Member #${m.member_number}`) })).filter((m) => m.num !== state.memberNumber && (!query || m.name.toLowerCase().includes(query))).slice(0, 8) : [];
        if (members.length) {
          closeMentionPanel();
      ...
      el(
        ...
        el(
          ...
          el("button", {
            ...
            onclick: () => {
              const input2 = list.querySelector(".bcm-add-contact-input");
              doAddContact(input2?.value.trim());
            }
          }, "Start Chat"),
          el("button", {
            style: {
  ...
  async function openJoinGroupDialog() {
    ...
    try {
      ...
      if (r?.alreadyMember) {
        ...
        showNote(`\u{1F4E8} Join request sent to "${preview.groupName}" \u2014 an admin will review it`, false);
      }
    } catch (ex) {
...
Continue without them?`);
  ...
  function onLogin() {
    ...
    if (!state.keyShortcutRegistered) {
      ...
      window.addEventListener("keydown", (e) => {
        ...
        if (mod2 && e.key === "k") {
          e.preventDefault();
          state.dialogEl?.querySelector(".bcm-search")?.focus();
          return;
        }
        if (mod2 && e.key === "n" && !e.shiftKey) {
          e.preventDefault();
          promptAddContact();
          return;
        }
        ...
        if ((mod2 || e.ctrlKey) && e.key === "Enter" && isInput) {
          e.preventDefault();
          sendMessage();
          return;
        }
    ...
    if (!GM_getValue(state.STORE + "onboarded", false)) {
      setTimeout(async () => {
        ...
        await openModal({
          ...
          body: el(
            "div",
            { style: { display: "flex", flexDirection: "column", gap: "8px", fontSize: "12px", color: "var(--bcm-text)" } },
    ...
    checkDndSchedule();
    setInterval(checkDndSchedule, 6e4);
    setInterval(() => {
    ...
    let prevRoomNums = /* @__PURE__ */ new Set();
    setInterval(() => {
      if (!state.loggedIn) return;
      try {
        const W = unsafeWindow;
  ...
  function pollForLogin() {
    const W = unsafeWindow;
    if (W.Player?.MemberNumber && !state.loggedIn) {
      onLogin();
      return;
    }
    setTimeout(pollForLogin, 1e3);
  }
...
IDB: ${getDb() ? "open" : "closed"}`);
  ...
  loadModSDK().then((sdk) => {
    try {
      if (sdk) {
        const mod2 = sdk.registerMod({
          name: "BCOfflineMessenger",
          fullName: "BC Offline Messenger",
          version: "1.0.0",
          repository: "https://github.com/khiles/BC-Messenger"
        });
        registerHooks(mod2);
      } else {
